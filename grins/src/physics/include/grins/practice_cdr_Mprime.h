#ifndef GRINS_PRAC_CDR_MPRIME_H
#define GRINS_PRAC_CDR_MPRIME_H
...
namespace GRINS{
	...
	class PracticeCDR_Mprime : public Physics{
	
	public:
	
		PracticeCDR_Mprime( const GRINS::PhysicsName& physics_name, const GetPot& input);
		~PracticeCDR_Mprime();
	...
	protected:
		...
		//indices for auxillary variables
		VariableIndex _aux_c_var, _aux_zc_var, _aux_fc_var;
		
		GRINSEnums::FEFamily _fefamily; //element type
	
	private:
	
		PracticeCDR_Mprime(); //not sure what the point of this is, but the others have it...
