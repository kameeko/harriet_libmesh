...
namespace GRINS{
	...
	PracticeConvDiff::PracticeConvDiff( const GRINS::PhysicsName& physics_name, const GetPot& input )
		...
		std::string find_velocity_here = input("Physics/PracticeConvectionDiffusion/velocity_file","vels0.txt");
    ...
    for (unsigned int qp=0; qp != n_qpoints; qp++){
	    ...
	    libMesh::NumberVectorValue U     (u,     v);
			...
			for (unsigned int i=0; i != n_c_dofs; i++){
				Fc(i) += JxW[qp] *(_k*(dphi[i][qp]*grad_c) + U*grad_c*phi[i][qp] - f*phi[i][qp]);  // diffusion term

				if (compute_jacobian){
					for (unsigned int j=0; j != n_c_dofs; j++){
						Kcc(i,j) += JxW[qp]*( _k*(dphi[i][qp]*dphi[j][qp]) + U*dphi[j][qp]*phi[i][qp]); // diffusion term
