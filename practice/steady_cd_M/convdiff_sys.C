	...
	for (unsigned int qp=0; qp != n_qpoints; qp++)
	  {
	    ...
	    for (unsigned int i=0; i != n_c_dofs; i++){ 
	      Rauxc(i) += JxW[qp]*(-(1/params[0])*grad_zc*dpsi[i][qp] + U*grad_zc*psi[i][qp]);
	      Rauxzc(i) += JxW[qp]*(-(1/params[0])*grad_c*dpsi[i][qp] - U*grad_c*psi[i][qp] + fc*psi[i][qp]);
	      if(regtype == 0)
	      	Rauxfc(i) += JxW[qp]*(beta*fc*psi[i][qp] + zc*psi[i][qp]);
	     	else if(regtype == 1)
	     		Rauxfc(i) += JxW[qp]*(beta*grad_fc*dpsi[i][qp] + zc*psi[i][qp]);
	     		
	      Rc(i) += JxW[qp]*(-(1/params[0])*grad_auxzc*dpsi[i][qp] + U*grad_auxzc*psi[i][qp] + auxc*psi[i][qp]);
	      if(fabs(ptx - 0.5) <= 0.125 && fabs(pty - 0.5) <= 0.125) //is this correct?
     			Rc(i) += JxW[qp]*psi[i][qp]; //Rc(i) += JxW[qp]?
	      Rzc(i) += JxW[qp]*(-(1/params[0])*grad_auxc*dpsi[i][qp] - U*grad_auxc*psi[i][qp] + auxfc*psi[i][qp]);
	     	if(regtype == 0)
