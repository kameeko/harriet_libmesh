...
void NavierStokesConvDiffSys::init_data (){
...
void NavierStokesConvDiffSys::init_context(DiffContext &context){
...
bool NavierStokesConvDiffSys::element_time_derivative (bool request_jacobian, DiffContext& context){
	...
	for (unsigned int qp=0; qp != n_qpoints; qp++)
	  {
	    ...
	    const Real rho = 1.0; //scaling on Navier-Stoke's extra term...

	    // First, an i-loop over the velocity degrees of freedom.
	    // We know that n_u_dofs == n_v_dofs so we can compute contributions
	    // for both at the same time.
	    for (unsigned int i=0; i != n_u_dofs; i++){ 
	      Fu(i) += JxW[qp] * (-rho*U*grad_u*phi[i][qp] + p*dphi[i][qp](0) - (grad_u*dphi[i][qp]));
	      Fv(i) += JxW[qp] * (-rho*U*grad_v*phi[i][qp] + p*dphi[i][qp](1) - (grad_v*dphi[i][qp]));
	      ...
	      if (request_jacobian && ctxt.elem_solution_derivative){
	        ...
	        for (unsigned int j=0; j != n_u_dofs; j++){ 
	        	Kuu(i,j) += JxW[qp]*(-rho*U*dphi[j][qp]*phi[i][qp] - rho*phi[j][qp]*u_x*phi[i][qp] - dphi[j][qp]*dphi[i][qp]);
	        	Kuv(i,j) += JxW[qp]*(-rho*phi[j][qp]*u_y*phi[i][qp]);
	        	Kvu(i,j) += JxW[qp]*(-rho*phi[j][qp]*v_x*phi[i][qp]);
	        	Kvv(i,j) += JxW[qp]*(-rho*U*dphi[j][qp]*phi[i][qp] - rho*phi[j][qp]*v_y*phi[i][qp] - dphi[j][qp]*dphi[i][qp]);
	        }
...
void NavierStokesConvDiffSys::postprocess (){
	//const unsigned int dim = this->get_mesh().mesh_dimension();
	...
	Number ppin = point_value(p_var, Point(0.,0.));
	std::cout << "origin pressure: " << ppin << std::endl;
	output.close();
...
Point NavierStokesConvDiffSys::forcing(const Point& pt){
	Point f;
	f(0) = exp(-10*(pow(pt(0)-0.25,2)+pow(pt(1)-0.25,2)));
	return f;
}

bool NavierStokesConvDiffSys::side_constraint (bool request_jacobian,
                                    DiffContext &context)
{
  FEMContext &ctxt = cast_ref<FEMContext&>(context);
