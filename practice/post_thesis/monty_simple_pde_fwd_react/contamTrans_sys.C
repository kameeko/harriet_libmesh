...
bool ContamTransSys::side_time_derivative(bool request_jacobian, DiffContext & context)
{
  ...
  for (unsigned int qp=0; qp != n_qpoints; qp++)
  {
    ...
    for (unsigned int i=0; i != n_c_dofs; i++)
    {
      ...
      {
        R(i) += JxW[qp]*(U*face_normals[qp]*c - bsource*U(0))*phi[i][qp];
      }
/*      
      if(isUp 
      ...
      {
        R(i) += JxW[qp]*(U*face_normals[qp]*c + source_rate*source_conc/(source_vol*water_density)*vx)*phi[i][qp]; //why vx? doesn't that point the wrong way?
      }
*/      
