...
int main (int argc, char** argv){
  ...
  LinearImplicitSystem & system =
    equation_systems.add_system<TransientLinearImplicitSystem> ("StokesConvDiff");
  ...
  const Real nonlinear_tolerance       = 1.e-3;
	...
	TransientLinearImplicitSystem & stokes_cd_system =
    equation_systems.get_system<TransientLinearImplicitSystem> ("StokesConvDiff");
  ...
  stokes_cd_system.time = 0.0;
  const unsigned int n_timesteps = 15;
	
	//get copy of solution at current nonlinear iteration
	AutoPtr<NumericVector<Number> >
		last_nonlinear_soln(stokes_cd_system.solution->clone());

  equation_systems.print_info();
  
  // Create a performance-logging object for this example
  PerfLog perf_log("Unsteady Coupled Stokes-Convection-Diffusion");

	for(unsigned int t_step=0; t_step<n_timesteps; ++t_step){
		stokes_cd_system.time += dt; //advance time
		
		//update message
		std::cout << "\n\n*** Solving time step" << t_step <<
									", time = " << stokes_cd_system.time <<
									" ***" << std::endl;
		
		//update solution from previous time step
		*stokes_cd_system.old_local_solution = *stokes_cd_system.current_local_solution;

		//reset linear solver tolerance to "reasonable" starting value
		const Real initial_linear_solver_tol = 1.0e-6;
		...
		for(unsigned int l=0; l<n_nonlinear_steps; ++l){
		  ...
		  if(isnan(norm_delta))
		  	std::cout << "DON'T FORGET TO ADD ARGUMENT --use-laspack\n";
		  ...
		  //otherwise, decrease linear system tolerance
              ...
              for (unsigned int ns=0; ns<side->n_nodes(); ns++)
                {
                  ...
                  const Real penalty = 1.e10;
									...
									//time-dependent lid-cavity boundary conditions
									Real PI = 3.14159265358979323846;
                  const Real u_value = (yf > .99) ? (1. + 0.5*std::sin(10.0*PI*system.time)) : 0.;
