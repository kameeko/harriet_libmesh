...
bool NavStokesConvDiffSys::element_time_derivative (bool request_jacobian, DiffContext& context){
  ...
  ctxt.get_element_fe( p_var, p_elem_fe );
	...
	DenseSubMatrix<Number> &J_auxu_zv = ctxt.get_elem_jacobian(aux_u_var, zv_var);
	DenseSubMatrix<Number> &J_auxu_zc = ctxt.get_elem_jacobian(aux_u_var, zc_var);
	DenseSubMatrix<Number> &J_auxu_auxu = ctxt.get_elem_jacobian(aux_u_var, aux_u_var);
	DenseSubMatrix<Number> &J_auxu_auxv = ctxt.get_elem_jacobian(aux_u_var, aux_v_var);
	DenseSubMatrix<Number> &J_auxu_auxp = ctxt.get_elem_jacobian(aux_u_var, aux_p_var);
	DenseSubMatrix<Number> &J_auxu_auxc = ctxt.get_elem_jacobian(aux_u_var, aux_c_var);
	DenseSubMatrix<Number> &J_auxu_auxzu = ctxt.get_elem_jacobian(aux_u_var, aux_zu_var);
	DenseSubMatrix<Number> &J_auxu_auxzv = ctxt.get_elem_jacobian(aux_u_var, aux_zv_var);
	DenseSubMatrix<Number> &J_auxu_auxzc = ctxt.get_elem_jacobian(aux_u_var, aux_zc_var);
	...
	DenseSubMatrix<Number> &J_auxv_zu = ctxt.get_elem_jacobian(aux_v_var, zu_var);
	...
	DenseSubMatrix<Number> &J_auxv_auxu = ctxt.get_elem_jacobian(aux_v_var, aux_u_var);
	DenseSubMatrix<Number> &J_auxv_auxv = ctxt.get_elem_jacobian(aux_v_var, aux_v_var);
	DenseSubMatrix<Number> &J_auxv_auxp = ctxt.get_elem_jacobian(aux_v_var, aux_p_var);
	DenseSubMatrix<Number> &J_auxv_auxc = ctxt.get_elem_jacobian(aux_v_var, aux_c_var);
	DenseSubMatrix<Number> &J_auxv_auxzu = ctxt.get_elem_jacobian(aux_v_var, aux_zu_var);
	DenseSubMatrix<Number> &J_auxv_auxzv = ctxt.get_elem_jacobian(aux_v_var, aux_zv_var);
	DenseSubMatrix<Number> &J_auxv_auxzc = ctxt.get_elem_jacobian(aux_v_var, aux_zc_var);
	
	DenseSubMatrix<Number> &J_auxp_auxu = ctxt.get_elem_jacobian(aux_p_var, aux_u_var);
	DenseSubMatrix<Number> &J_auxp_auxv = ctxt.get_elem_jacobian(aux_p_var, aux_v_var);
	...
	DenseSubMatrix<Number> &J_auxc_c = ctxt.get_elem_jacobian(aux_c_var, c_var); //after adding QoI stuff...
	DenseSubMatrix<Number> &J_auxc_zc = ctxt.get_elem_jacobian(aux_c_var, zc_var);
	DenseSubMatrix<Number> &J_auxc_auxc = ctxt.get_elem_jacobian(aux_c_var, aux_c_var);
	DenseSubMatrix<Number> &J_auxc_auxzu = ctxt.get_elem_jacobian(aux_c_var, aux_zu_var);
	DenseSubMatrix<Number> &J_auxc_auxzv = ctxt.get_elem_jacobian(aux_c_var, aux_zv_var);
	DenseSubMatrix<Number> &J_auxc_auxzc = ctxt.get_elem_jacobian(aux_c_var, aux_zc_var);
	...
	DenseSubMatrix<Number> &J_auxzu_auxzu = ctxt.get_elem_jacobian(aux_zu_var, aux_zu_var);
	DenseSubMatrix<Number> &J_auxzu_auxzp = ctxt.get_elem_jacobian(aux_zu_var, aux_zp_var);
	...
	DenseSubMatrix<Number> &J_auxzv_auxzv = ctxt.get_elem_jacobian(aux_zv_var, aux_zv_var);
	DenseSubMatrix<Number> &J_auxzv_auxzp = ctxt.get_elem_jacobian(aux_zv_var, aux_zp_var);
	
	DenseSubMatrix<Number> &J_auxzp_auxzu = ctxt.get_elem_jacobian(aux_zp_var, aux_zu_var);
	DenseSubMatrix<Number> &J_auxzp_auxzv = ctxt.get_elem_jacobian(aux_zp_var, aux_zv_var);
	...
	DenseSubMatrix<Number> &J_auxzc_auxzu = ctxt.get_elem_jacobian(aux_zc_var, aux_zu_var);
	DenseSubMatrix<Number> &J_auxzc_auxzv = ctxt.get_elem_jacobian(aux_zc_var, aux_zv_var);
	DenseSubMatrix<Number> &J_auxzc_auxzc = ctxt.get_elem_jacobian(aux_zc_var, aux_zc_var);
	DenseSubMatrix<Number> &J_auxzc_auxfc = ctxt.get_elem_jacobian(aux_zc_var, aux_fc_var);
	
	DenseSubMatrix<Number> &J_auxfc_auxc = ctxt.get_elem_jacobian(aux_fc_var, aux_c_var);
	DenseSubMatrix<Number> &J_auxfc_auxfc = ctxt.get_elem_jacobian(aux_fc_var, aux_fc_var);
	...
	/*if(request_jacobian){
	DenseMatrix<Number> meep(ctxt.get_elem_jacobian()); //DEBUG
	std::cout << qpoint[0](0) << " " << qpoint[0](1) << " " << meep.l1_norm() << std::endl; //DEBUG 
	}*/
