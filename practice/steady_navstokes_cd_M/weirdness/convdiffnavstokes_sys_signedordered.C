...
bool NavStokesConvDiffSys::element_time_derivative (bool request_jacobian, DiffContext& context){
  ...
  ctxt.get_element_fe( p_var, p_elem_fe );
	...
	DenseSubMatrix<Number> &J_u_auxp = ctxt.get_elem_jacobian(u_var, aux_p_var);
	...
	DenseSubMatrix<Number> &J_v_auxp = ctxt.get_elem_jacobian(v_var, aux_p_var);
	...
	DenseSubMatrix<Number> &J_p_auxu = ctxt.get_elem_jacobian(p_var, aux_u_var);
	DenseSubMatrix<Number> &J_p_auxv = ctxt.get_elem_jacobian(p_var, aux_v_var);
	...
	DenseSubMatrix<Number> &J_c_c = ctxt.get_elem_jacobian(c_var, c_var); //after adding QoI stuff...
	...
	DenseSubMatrix<Number> &J_c_auxzu = ctxt.get_elem_jacobian(c_var, aux_zu_var);
	DenseSubMatrix<Number> &J_c_auxzv = ctxt.get_elem_jacobian(c_var, aux_zv_var);
	...
	DenseSubMatrix<Number> &J_zu_auxzu = ctxt.get_elem_jacobian(zu_var, aux_zu_var);
	DenseSubMatrix<Number> &J_zu_auxzp = ctxt.get_elem_jacobian(zu_var, aux_zp_var);
	...
	DenseSubMatrix<Number> &J_zv_auxzv = ctxt.get_elem_jacobian(zv_var, aux_zv_var);
	DenseSubMatrix<Number> &J_zv_auxzp = ctxt.get_elem_jacobian(zv_var, aux_zp_var);
	
	DenseSubMatrix<Number> &J_zp_auxzu = ctxt.get_elem_jacobian(zp_var, aux_zu_var);
	DenseSubMatrix<Number> &J_zp_auxzv = ctxt.get_elem_jacobian(zp_var, aux_zv_var);
	...
	DenseSubMatrix<Number> &J_zc_auxzu = ctxt.get_elem_jacobian(zc_var, aux_zu_var);
	DenseSubMatrix<Number> &J_zc_auxzv = ctxt.get_elem_jacobian(zc_var, aux_zv_var);
	DenseSubMatrix<Number> &J_zc_auxzc = ctxt.get_elem_jacobian(zc_var, aux_zc_var);
	DenseSubMatrix<Number> &J_zc_auxfc = ctxt.get_elem_jacobian(zc_var, aux_fc_var);
	
	DenseSubMatrix<Number> &J_fc_auxc = ctxt.get_elem_jacobian(fc_var, aux_c_var);
	...
	for (unsigned int qp=0; qp != n_qpoints; qp++)
	  {
	    ...
	    for (unsigned int i=0; i != n_u_dofs; i++){ 
	      ...
	      Rauxv(i) += JxW[qp]*(params[1]*grad_zv*dphi[i][qp] - zp*dphi[i][qp](1) - zc*c_y*phi[i][qp]
	     	...
	     	Ru(i) += JxW[qp]*(params[0]*(-U*grad_auxu - v_y*auxu + auxv*v_x - auxzu*zu_x + zv*auxzv_x)*phi[i][qp] 
	     											+ params[1]*grad_auxu*dphi[i][qp]
	     											- auxp*dphi[i][qp](0) - auxc*c_x*phi[i][qp] - zc*auxzc_x*phi[i][qp]);
	     	Rv(i) += JxW[qp]*(params[0]*(-U*grad_auxv - u_x*auxv + auxu*u_y - auxzv*zv_y + zu*auxzu_y)*phi[i][qp]
	     											+ params[1]*grad_auxv*dphi[i][qp]
	     											- auxp*dphi[i][qp](1) - auxc*c_y*phi[i][qp] - zc*auxzc_y*phi[i][qp]);
	     	Rzu(i) += JxW[qp]*(params[0]*(U*grad_auxzu + u_x*auxzu)*phi[i][qp]
	     											+ params[1]*grad_auxzu*dphi[i][qp] - auxzp*dphi[i][qp](0));
	     	Rzv(i) += JxW[qp]*(params[0]*(U*grad_auxzv + v_y*auxzv)*phi[i][qp]
	      ...
	      if (request_jacobian && ctxt.elem_solution_derivative){
	        ...
	        for (unsigned int j=0; j != n_u_dofs; j++){ 
	        	...
	        	J_auxv_zv(i,j) += JxW[qp]*(params[1]*dphi[j][qp]*dphi[i][qp]
	     			...
	     			J_u_u(i,j) += -JxW[qp]*params[0]*phi[j][qp]*auxu_x*phi[i][qp];
	     			J_u_v(i,j) += -JxW[qp]*params[0]*(phi[j][qp]*auxu_y + dphi[j][qp](1)*auxu - dphi[j][qp](0)*auxv)*phi[i][qp];
	     			J_u_zu(i,j) += JxW[qp]*params[0]*(-auxzu*dphi[j][qp](0))*phi[i][qp];
	     			J_u_zv(i,j) += JxW[qp]*params[0]*auxzv_x*phi[j][qp]*phi[i][qp];
	     			J_u_auxu(i,j) += -JxW[qp]*(params[0]*(U*dphi[j][qp] + v_y*phi[j][qp])*phi[i][qp] 
	     														- params[1]*dphi[j][qp]*dphi[i][qp]);
	     			J_u_auxv(i,j) += -JxW[qp]*params[0]*(-phi[j][qp]*v_x)*phi[i][qp];
	     			J_u_auxzu(i,j) += JxW[qp]*params[0]*(-phi[j][qp]*zu_x)*phi[i][qp];
	     			J_u_auxzv(i,j) += JxW[qp]*params[0]*(zv*dphi[j][qp](0))*phi[i][qp];
	     			
	     			J_v_u(i,j) += -JxW[qp]*params[0]*(phi[j][qp]*auxv_x + dphi[j][qp](0)*auxv - dphi[j][qp](1)*auxu)*phi[i][qp];
	     			J_v_v(i,j) += -JxW[qp]*params[0]*phi[j][qp]*auxv_y*phi[i][qp];
	     			J_v_zu(i,j) += JxW[qp]*params[0]*auxzu_y*phi[j][qp]*phi[i][qp];
	     			J_v_zv(i,j) += JxW[qp]*params[0]*(-auxzv*dphi[j][qp](1))*phi[i][qp];
	     			J_v_auxu(i,j) += -JxW[qp]*params[0]*(-u_y*phi[j][qp])*phi[i][qp];
	     			J_v_auxv(i,j) += -JxW[qp]*(params[0]*(U*dphi[j][qp] + u_x*phi[j][qp])*phi[i][qp]
	     														- params[1]*dphi[j][qp]*dphi[i][qp]);
	     			J_v_auxzu(i,j) += JxW[qp]*params[0]*zu*dphi[j][qp](1)*phi[i][qp];
	     			J_v_auxzv(i,j) += JxW[qp]*params[0]*(-zv_y*phi[j][qp])*phi[i][qp];
	     											
	     			J_zu_u(i,j) += JxW[qp]*params[0]*(phi[j][qp]*auxzu_x + dphi[j][qp](0)*auxzu)*phi[i][qp];
	     			J_zu_v(i,j) += JxW[qp]*params[0]*phi[j][qp]*auxzu_y*phi[i][qp];
	     			J_zu_auxzu(i,j) += JxW[qp]*(params[0]*(U*dphi[j][qp] + u_x*phi[j][qp])*phi[i][qp]
	     															+ params[1]*dphi[j][qp]*dphi[i][qp]);
	     											
	     			J_zv_u(i,j) += JxW[qp]*params[0]*phi[j][qp]*auxzv_x*phi[i][qp];
	     			J_zv_v(i,j) += JxW[qp]*params[0]*(phi[j][qp]*auxzv_y + dphi[j][qp](1)*auxzv)*phi[i][qp];
	     			J_zv_auxzv(i,j) += JxW[qp]*(params[0]*(U*dphi[j][qp] + v_y*phi[j][qp])*phi[i][qp]
	     															+ params[1]*dphi[j][qp]*dphi[i][qp]);								
	        ...
	        for (unsigned int j=0; j != n_p_dofs; j++){
	          ...
	          J_u_c(i,j) += -JxW[qp]*auxc*dpsi[j][qp](0)*phi[i][qp];
	          J_u_zc(i,j) += -JxW[qp]*auxzc_x*psi[j][qp]*phi[i][qp];
	          J_u_auxc(i,j) += -JxW[qp]*c_x*psi[j][qp]*phi[i][qp];
	          J_u_auxp(i,j) += -JxW[qp]*psi[j][qp]*dphi[i][qp](0);
	          J_u_auxzc(i,j) += -JxW[qp]*zc*dpsi[j][qp](0)*phi[i][qp];
							
						J_v_c(i,j) += -JxW[qp]*auxc*dpsi[j][qp](1)*phi[i][qp];
						J_v_zc(i,j) += -JxW[qp]*auxzc_y*psi[j][qp]*phi[i][qp];
						J_v_auxc(i,j) += -JxW[qp]*psi[j][qp]*c_y*phi[i][qp];
						J_v_auxp(i,j) += -JxW[qp]*psi[j][qp]*dphi[i][qp](1);
						J_v_auxzc(i,j) += -JxW[qp]*zc*dpsi[j][qp](1)*phi[i][qp];

						J_zu_auxzp(i,j) += JxW[qp]*(-psi[j][qp])*dphi[i][qp](0);
						J_zv_auxzp(i,j) += JxW[qp]*(-psi[j][qp])*dphi[i][qp](1);
	        }
	      }
	    }
	    ...
	    for (unsigned int i=0; i != n_p_dofs; i++){ 
	      ...
	      Rauxp(i) += JxW[qp]*(zu*dpsi[i][qp](0) + zv*dpsi[i][qp](1));
	      ...
	      if(regtype == 0)
	     	...
	     	Rp(i) += -JxW[qp]*(-auxu*dpsi[i][qp](0) - auxv*dpsi[i][qp](1));
	     	Rc(i) += JxW[qp]*(-grad_auxc*dpsi[i][qp] + (U*grad_auxc + auxc*(u_x + v_y))*psi[i][qp]
     									+ (zc*auxzu_x + zc_x*auxzu + zc*auxzv_y + zc_y*auxzv)*psi[i][qp]
     									+ auxzc*psi[i][qp]);
     		if(fabs(ptx - 0.5) <= 0.125 && fabs(pty - 0.5) <= 0.125)
     			Rc(i) += JxW[qp]*psi[i][qp];/////////////////////////////////////
	     	Rzp(i) += JxW[qp]*(auxzu*dpsi[i][qp](0) + auxzv*dpsi[i][qp](1));
	     	Rzc(i) += JxW[qp]*((-auxzu*c_x - auxzv*c_y + auxfc - U*grad_auxzc)*psi[i][qp]
	     								- grad_auxzc*dpsi[i][qp]);
	     	if(regtype == 0)
	     		Rfc(i) += JxW[qp]*((auxc + beta*auxfc)*psi[i][qp]);
	     	else if(regtype == 1)
	     		Rfc(i) += JxW[qp]*(auxc*psi[i][qp] + beta*grad_auxfc*dpsi[i][qp]);
	      ...
	      if (request_jacobian && ctxt.elem_solution_derivative){
	        for(unsigned int j=0; j != n_u_dofs; j++){
	          J_auxzp_u(i,j) += -JxW[qp]*(dphi[j][qp](0)*psi[i][qp]);
	        	...
	        	J_auxp_zu(i,j) += JxW[qp]*(phi[j][qp]*dpsi[i][qp](0));
	        	J_auxp_zv(i,j) += JxW[qp]*(phi[j][qp]*dpsi[i][qp](1));
	        	...
	        	J_p_auxu(i,j) += JxW[qp]*(phi[j][qp]*dpsi[i][qp](0));
	        	J_p_auxv(i,j) += JxW[qp]*(phi[j][qp]*dpsi[i][qp](1));
	        	
	        	J_c_u(i,j) += -JxW[qp]*(-phi[j][qp]*auxc_x - auxc*dphi[j][qp](0))*psi[i][qp];
	        	J_c_v(i,j) += -JxW[qp]*(-phi[j][qp]*auxc_y - auxc*dphi[j][qp](1))*psi[i][qp];
	        	J_c_auxzu(i,j) += JxW[qp]*(zc*dphi[j][qp](0) + zc_x*phi[j][qp])*psi[i][qp];
	        	J_c_auxzv(i,j) += JxW[qp]*(zc*dphi[j][qp](1) + zc_y*phi[j][qp])*psi[i][qp];
	        	
	        	J_zp_auxzu(i,j) += JxW[qp]*phi[j][qp]*dpsi[i][qp](0);
	        	J_zp_auxzv(i,j) += JxW[qp]*phi[j][qp]*dpsi[i][qp](1);	
	     								      	
	        	J_zc_u(i,j) += -JxW[qp]*phi[j][qp]*auxzc_x*psi[i][qp];
	        	J_zc_v(i,j) += -JxW[qp]*phi[j][qp]*auxzc_y*psi[i][qp];
	        	J_zc_auxzu(i,j) += JxW[qp]*(-phi[j][qp]*c_x)*psi[i][qp];
	        	J_zc_auxzv(i,j) += JxW[qp]*(-phi[j][qp]*c_y)*psi[i][qp];
	       	...
	       	for(unsigned int j=0; j != n_p_dofs; j++){
	       		...
	       		if(regtype == 0)
     				...
     				if(fabs(ptx - 0.5) <= 0.125 && fabs(pty - 0.5) <= 0.125)					
						...
						J_c_zc(i,j) += JxW[qp]*(psi[j][qp]*(auxzu_x + auxzv_y) 
																+ dpsi[j][qp](0)*auxzu + dpsi[j][qp](1)*auxzv)*psi[i][qp];
						J_c_auxc(i,j) += JxW[qp]*(-dpsi[j][qp]*dpsi[i][qp]
																+ (U*dpsi[j][qp] + psi[j][qp]*(u_x + v_y))*psi[i][qp]);
						J_c_auxzc(i,j) += -JxW[qp]*(-psi[j][qp]*psi[i][qp]);
	     													
						J_zc_c(i,j) += JxW[qp]*(-auxzu*dpsi[j][qp](0) - auxzv*dpsi[j][qp](1))*psi[i][qp];
						J_zc_auxzc(i,j) += -JxW[qp]*(U*dpsi[j][qp]*psi[i][qp] + dpsi[j][qp]*dpsi[i][qp]);
						J_zc_auxfc(i,j) += JxW[qp]*psi[j][qp]*psi[i][qp];
	     		
						if(regtype == 0){
			      	J_fc_auxc(i,j) += -JxW[qp]*(-psi[j][qp])*psi[i][qp];
	  ...
	  for(unsigned int dnum=0; dnum<datavals.size(); dnum++){
        ...
        for (unsigned int i=0; i != n_p_dofs; i++){
  	  		Rauxc(i) += -(cstar - cpred)*point_phi[i];
  ...
  if( ctxt.get_elem().contains_point(zero))
    {
      ...
      DenseSubMatrix<Number> &Jpap = ctxt.get_elem_jacobian( p_var, aux_p_var );
      DenseSubMatrix<Number> &Jzpazp = ctxt.get_elem_jacobian( zp_var, aux_zp_var );
      DenseSubMatrix<Number> &Japzp = ctxt.get_elem_jacobian( aux_p_var, zp_var );
      DenseSubMatrix<Number> &Jazpp = ctxt.get_elem_jacobian( aux_zp_var, p_var );
      ...
      for (unsigned int i=0; i != n_p_dofs; i++)
        {
          Razp(i) += penalty * (p - p_pin) * point_phi[i];
          Rap(i) += penalty*(zp - zp_pin)*point_phi[i];
          Rp(i) += penalty*(auxp - auxp_pin)*point_phi[i];
          Rzp(i) += penalty*(auxzp - auxzp_pin)*point_phi[i];
          if (request_jacobian && ctxt.get_elem_solution_derivative())
            {
              libmesh_assert_equal_to (ctxt.get_elem_solution_derivative(), 1.0);

              for (unsigned int j=0; j != n_p_dofs; j++){
                Jazpp(i,j) += penalty * point_phi[i] * point_phi[j];
                Japzp(i,j) += penalty * point_phi[i] * point_phi[j];
                Jpap(i,j) += penalty * point_phi[i] * point_phi[j];
                Jzpazp(i,j) += penalty * point_phi[i] * point_phi[j];
              }
            }
        }
    }
