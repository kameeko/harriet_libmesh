	...
	for (unsigned int qp=0; qp != n_qpoints; qp++)
	  {
	    ...
	    Gradient 
	  	...
	  	const Real pty = (dim == 2) ? qpoint[qp](1) : 0.0;
			...
			Real u = 2.0*pty*(1.0-pty);
			Real v = 0.0;

	    NumberVectorValue U(u,v);
	    
	    //Real R = Rcoeff; //reaction coefficient
	    Real R = -(*continuation_parameter); //reaction coefficient
			...
			for (unsigned int i=0; i != n_c_dofs; i++){
	          ...
	          Rc(i) += JxW[qp]*(2*zc*c*phi[i][qp]);
	          Rzc(i) += JxW[qp]*(c*c*phi[i][qp]);
	          Rfc(i) += 0.; 
	          break;
	        }
	      }
