...
int main(int argc, char** argv){
  ...
  ConvDiff_MprimeSys & system_mix = 
		equation_systems_mix.add_system<ConvDiff_MprimeSys>("Diff_ConvDiff_MprimeSys");
    ...
    if (a_step == max_adaptivesteps)
      {
      	...
      	using std::chrono::duration_cast;
    		...
    		typedef std::chrono::high_resolution_clock clock;
				...
				auto start_primary = clock::now();
        system_primary.solve();
        auto end_primary = clock::now();
				...
				auto start_aux = clock::now();
				system_aux.solve();
				auto end_aux = clock::now();
				
				std::cout << "\n\n Residual L2 norm (auxiliary): " << system_aux.calculate_norm(*system_aux.rhs, L2) << "\n";
				
        std::cout << "\n  Primary solve took " << 
        	duration_cast<milliseconds>(end_primary-start_primary).count() << " ms" << std::endl;
        std::cout << "  Auxiliary solve took " << 
        	duration_cast<milliseconds>(end_aux-start_aux).count() << " ms" << std::endl;
        std::cout << "  Total solve time " << 
        	duration_cast<milliseconds>(end_aux-start_aux).count()
        	+duration_cast<milliseconds>(end_primary-start_primary).count() << " ms\n" << std::endl;
