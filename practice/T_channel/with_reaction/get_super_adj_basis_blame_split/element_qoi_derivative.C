...
void ConvDiff_PrimarySys::element_qoi_derivative (DiffContext &context,
                                            const QoISet & /* qois */)
{
  const unsigned int dim = this->get_mesh().mesh_dimension();
  ...
  // Fill the QoI RHS corresponding to this QoI. Since this is the 0th QoI
  // we fill in the [0][i] subderivatives, i corresponding to the variable index.
  DenseSubVector<Number> &Qauxc = ctxt.get_qoi_derivatives(0, c_var);
  DenseSubVector<Number> &Qauxzc = ctxt.get_qoi_derivatives(0, zc_var);
  DenseSubVector<Number> &Qauxfc = ctxt.get_qoi_derivatives(0, fc_var);
...
void ConvDiff_AuxSys::element_qoi_derivative (DiffContext &context,
                                            const QoISet & /* qois */)
{
  ...
  // Fill the QoI RHS corresponding to this QoI. Since this is the 0th QoI
  // we fill in the [0][i] subderivatives, i corresponding to the variable index.
  DenseSubVector<Number> &Qc = ctxt.get_qoi_derivatives(0, aux_c_var);
  DenseSubVector<Number> &Qzc = ctxt.get_qoi_derivatives(0, aux_zc_var);
  DenseSubVector<Number> &Qfc = ctxt.get_qoi_derivatives(0, aux_fc_var);
