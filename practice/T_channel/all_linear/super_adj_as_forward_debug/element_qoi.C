...

{
  ...
  for (unsigned int qp = 0; qp != n_qpoints; qp++)
    {
      ...
      if((dim == 2 && fabs(ptx - 0.5) <= 0.125 && fabs(pty - 0.5) <= 0.125) 
        ...
        MLF_psiLF_elem += JxW[qp] * c;
	      ...
	      c_stash[myElemID].push_back(c);
	      zc_stash[myElemID].push_back(zc);
	      fc_stash[myElemID].push_back(fc);
	      auxc_stash[myElemID].push_back(auxc);
	      auxzc_stash[myElemID].push_back(auxzc);
	      auxfc_stash[myElemID].push_back(auxfc);
	      
	      gradc_stash[myElemID].push_back(grad_c);
	      gradzc_stash[myElemID].push_back(grad_zc);
	      gradfc_stash[myElemID].push_back(grad_fc);
	      gradauxc_stash[myElemID].push_back(grad_auxc);
	      gradauxzc_stash[myElemID].push_back(grad_auxzc);
	      gradauxfc_stash[myElemID].push_back(grad_auxfc);
	      //std::cout << "\nNOT UP TO DATE!!!!!!!\n";
		}
  ...
  for(unsigned int dnum=0; dnum<datavals.size(); dnum++){
    	...
    	Number c_point = ctxt.point_value(c_var, data_point);   

		if(debug_step == 0)
	  	MHF_psiLF_elem += (cpred - cstar)*auxc_point;
	  else if(debug_step == 1){
	  	auxc_point_stash[dnum] = auxc_point;
	  	c_point_stash[dnum] = c_point;
	  }
	  //else if(debug_step == 2)
