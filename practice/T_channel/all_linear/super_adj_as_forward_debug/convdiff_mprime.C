	...
	for (unsigned int qp=0; qp != n_qpoints; qp++)
	  {
	    ...
	    Number 
	      c_forw = c_stash[myElemID][qp],
	      zc_forw = zc_stash[myElemID][qp],
	      fc_forw = fc_stash[myElemID][qp],
	      auxc_forw = auxc_stash[myElemID][qp],
	      auxzc_forw = auxzc_stash[myElemID][qp],
	      auxfc_forw = auxfc_stash[myElemID][qp];
	    Gradient 
	      grad_c_forw = gradc_stash[myElemID][qp],
	      grad_zc_forw = gradzc_stash[myElemID][qp],
	      grad_fc_forw = gradfc_stash[myElemID][qp],
	      grad_auxc_forw = gradauxc_stash[myElemID][qp],
	      grad_auxzc_forw = gradauxzc_stash[myElemID][qp],
	      grad_auxfc_forw = gradauxfc_stash[myElemID][qp];
	    ...
	    if(dim == 2)
	      ...
	      Rauxc(i) += JxW[qp]*(-k*grad_zc*dphi[i][qp] + U*grad_zc*phi[i][qp] //+ 2*R*zc*c*phi[i][qp]);
	      								-(-k*grad_zc_forw*dphi[i][qp] + U*grad_zc_forw*phi[i][qp]));
	      Rauxzc(i) += JxW[qp]*(-k*grad_c*dphi[i][qp] - U*grad_c*phi[i][qp] + R*c*c*phi[i][qp] + fc*phi[i][qp]//);
	      								-(-k*grad_c_forw*dphi[i][qp] - U*grad_c_forw*phi[i][qp] + fc_forw*phi[i][qp]));
	      if(dim == 2)
     			Rauxfc(i) += JxW[qp]*(beta*grad_fc*dphi[i][qp] + zc*phi[i][qp] //);
     										-(beta*grad_fc_forw*dphi[i][qp] + zc_forw*phi[i][qp]));
     		else if(dim == 1 && i == 0){
	      ...
	      Rc(i) += JxW[qp]*(-k*grad_auxzc*dphi[i][qp] + U*grad_auxzc*phi[i][qp] //+ 2*R*zc*auxc*phi[i][qp]); 
	      									-(-k*grad_auxzc_forw*dphi[i][qp] + U*grad_auxzc_forw*phi[i][qp]));
	      ...
	      Rzc(i) += JxW[qp]*(-k*grad_auxc*dphi[i][qp] - U*grad_auxc*phi[i][qp] + auxfc*phi[i][qp] //+ 2*R*c*auxc*phi[i][qp]);
	      							-(-k*grad_auxc_forw*dphi[i][qp] - U*grad_auxc_forw*phi[i][qp] + auxfc_forw*phi[i][qp]));
	      if(dim == 2)
     			Rfc(i) += JxW[qp]*(auxzc*phi[i][qp] + beta*grad_auxfc*dphi[i][qp] //);
     									-(auxzc_forw*phi[i][qp] + beta*grad_auxfc_forw*dphi[i][qp]));
     		else if(dim == 1 && i == 0){
	  ...
	  for(unsigned int dnum=0; dnum<datavals.size(); dnum++){
	  	...
	  	if(ctxt.get_elem().contains_point(data_point) && (accounted_for[dnum]>=ctxt.get_elem().id()) ){
	  		...
	  		Number cpoint_forw = c_point_stash[dnum];
	  		Number auxcpoint_forw = auxc_point_stash[dnum];
        ...
        for (unsigned int i=0; i != n_c_dofs; i++){
  	  		...
  	  		Rauxc(i) += cpred*point_phi[i] - ((cpoint_forw-cstar)*point_phi[i]);
  	  		Rc(i) += auxc_pointy*point_phi[i] - (auxcpoint_forw*point_phi[i]);
...
if(debug_step == 0){
  ...
  //DEBUGGING
	...
	c_stash.resize(this->get_mesh().n_elem()); gradc_stash.resize(this->get_mesh().n_elem());
	zc_stash.resize(this->get_mesh().n_elem()); gradzc_stash.resize(this->get_mesh().n_elem());
	fc_stash.resize(this->get_mesh().n_elem()); gradfc_stash.resize(this->get_mesh().n_elem());
	auxc_stash.resize(this->get_mesh().n_elem()); gradauxc_stash.resize(this->get_mesh().n_elem());
	auxzc_stash.resize(this->get_mesh().n_elem()); gradauxzc_stash.resize(this->get_mesh().n_elem());
	auxfc_stash.resize(this->get_mesh().n_elem()); gradauxfc_stash.resize(this->get_mesh().n_elem());
	FEMSystem::postprocess();
}
