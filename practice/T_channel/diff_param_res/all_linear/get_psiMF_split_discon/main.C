    ...
    if (a_step == max_adaptivesteps)
      {
        system_primary.solve();
  			...
  			sol_transfer.transfer(system_primary.variable(system_primary.variable_number("c")),
				...
				sol_transfer.transfer(system_primary.variable(system_primary.variable_number("fconst")),
  				system_mix.variable(system_mix.variable_number("fconst")));
				sol_transfer.transfer(system_aux.variable(system_aux.variable_number("aux_fconst")),
  				system_mix.variable(system_mix.variable_number("aux_fconst")));
  			std::cout << "c: " << system_mix.calculate_norm(*system_mix.solution, 0, L2) << " " 
				...
				std::cout << "fconst: " << system_mix.calculate_norm(*system_mix.solution, 6, L2) << " " 
													<< system_primary.calculate_norm(*system_primary.solution, 3, L2) << std::endl;
				std::cout << "aux_fconst: " << system_mix.calculate_norm(*system_mix.solution, 7, L2) << " " 
													<< system_aux.calculate_norm(*system_aux.solution, 3, L2) << std::endl;
