...
std::cout << "\n\n\n WHY ARE YOU HERE?!?!?\n\n" << std::endl;
  ...
  int subdomain = ctxt.get_elem().subdomain_id();
	...
	//DenseSubMatrix<Number> &J_zc_auxfpin = ctxt.get_elem_jacobian(zc_var, aux_fpin_var);
	...
	//DenseSubMatrix<Number> &J_fc_auxfpin = ctxt.get_elem_jacobian(fc_var, aux_fpin_var);
	
	//DenseSubMatrix<Number> &J_fpin_auxfpin = ctxt.get_elem_jacobian(fpin_var, aux_fpin_var);
	//DenseSubMatrix<Number> &J_fpin_auxzc = ctxt.get_elem_jacobian(fpin_var, aux_zc_var);
	...
	//DenseSubMatrix<Number> &J_auxzc_fpin = ctxt.get_elem_jacobian(aux_zc_var, fpin_var);
	...
	//DenseSubMatrix<Number> &J_auxfc_fpin = ctxt.get_elem_jacobian(aux_fc_var, fpin_var);
	
	//DenseSubMatrix<Number> &J_auxfpin_fpin = ctxt.get_elem_jacobian(aux_fpin_var, fpin_var);
	//DenseSubMatrix<Number> &J_auxfpin_zc = ctxt.get_elem_jacobian(aux_fpin_var, zc_var);
	...
	//DenseSubVector<Number> &Rauxfpin = ctxt.get_elem_residual( aux_fpin_var );
	...
	for (unsigned int qp=0; qp != n_qpoints; qp++)
	  {
	    ...
	    NumberVectorValue U(u,v);
			...
			double screw_mag = 1.e3; //how tightly to enforce fc=fpin
			double screw_grad = 1.e2; //how tightly to enforce grad_fc = 0
	      ...
	      if(subdomain == scalar_subdomain_id){
	      	//Rauxzc(i) += JxW[qp]*(-k*grad_c*dphi[i][qp] - U*grad_c*phi[i][qp] + R*c*c*phi[i][qp] + fpin*phi[i][qp]);
		 			//Rauxfc(i) += JxW[qp]*(screw_mag*(fpin - fc)*phi[i][qp] + screw_grad*grad_fc*dphi[i][qp]);
	      }
	      ...
	      if((qoi_option == 1 && 
     				...
     				if((qoi_option == 1 && 
						...
						if(subdomain == field_subdomain_id){
					  	...
					  	J_auxfc_fc(i,j) += JxW[qp]*(beta*dphi[j][qp]*dphi[i][qp] + beta*phi[j][qp]*phi[i][qp]);
							...
							//if(j == 0)
								//J_auxzc_fpin(i,j) += JxW[qp]*phi[i][qp];
								
							J_auxfc_fc(i,j) += JxW[qp]*(-screw_mag*phi[j][qp]*phi[i][qp] + screw_grad*dphi[j][qp]*dphi[i][qp]);
	     				//if(j == 0)
	     					//J_auxfc_fpin(i,j) += JxW[qp]*screw_mag*phi[i][qp];
		      	}
			...
			/*if(subdomain == scalar_subdomain_id){
				Rauxfpin(0) += JxW[qp]*(beta*fpin + zc);
				Rfpin(0) += JxW[qp]*(beta*auxfpin + auxzc);
				
				if(request_jacobian){
					J_auxfpin_fpin(0,0) += JxW[qp]*beta;
					J_fpin_auxfpin(0,0) += JxW[qp]*beta;
					for (unsigned int j=0; j != n_c_dofs; j++){
						J_auxfpin_zc(0,j) += JxW[qp]*(phi[j][qp]);
