...
void ConvDiff_InvSys::init_context(DiffContext &context){
...
bool ConvDiff_InvSys::element_time_derivative (bool request_jacobian, DiffContext& context){
	const unsigned int dim = this->get_mesh().mesh_dimension();
  ...
  int subdomain = ctxt.get_elem().subdomain_id();
	...
	//DenseSubMatrix<Number> &J_fpin_fc = ctxt.get_elem_jacobian(fpin_var, fc_var);
	DenseSubMatrix<Number> &J_fpin_zc = ctxt.get_elem_jacobian(fpin_var, zc_var); //DEBUG
	...
	for (unsigned int qp=0; qp != n_qpoints; qp++)
	  {
	    ...
	    NumberVectorValue U(u,v);
			...
			for (unsigned int i=0; i != n_c_dofs; i++){
				...
				if (request_jacobian){
					for (unsigned int j=0; j != n_c_dofs; j++){
	     			...
	     			//else if(subdomain == scalar_subdomain_id && j == 0)
	     			//	J_fc_fpin(i,j) += JxW[qp]*(beta*phi[i][qp]); 
			...
			if(subdomain == scalar_subdomain_id){
			 	//double eek = 1.0;
				//Rfpin(0) += JxW[qp]*(eek-fc)*(eek-fc);
				//J_fpin_fpin(0,0) += JxW[qp]*2.0*(fpin-fc);
					
				Rfpin(0) += JxW[qp]*(beta*fpin + zc);
				J_fpin_fpin(0,0) += JxW[qp]*beta;	
				if(request_jacobian){
					for (unsigned int j=0; j != n_c_dofs; j++){
							//J_fpin_fc(0,j) += -JxW[qp]*2.0*(eek-fc)*phi[j][qp];
...
void ConvDiff_InvSys::postprocess (){
