...
void ConvDiff_MprimeSys::init_data (){
	...
	//fconst_var = this->add_variable("fconst", static_cast<Order>(conc_p), fefamily, &active_subdom_scalar); //DEBUG
	//aux_fconst_var = this->add_variable("aux_fconst", static_cast<Order>(conc_p), fefamily, &active_subdom_scalar); //DEBUG 
	//fconst_var = fc_var; aux_fconst_var = aux_fc_var; //DEBUG
...
bool ConvDiff_MprimeSys::element_time_derivative (bool request_jacobian, DiffContext& context){
  ...
  int subdomain = ctxt.get_elem().subdomain_id();
	...
	DenseSubMatrix<Number> &J_zc_auxfconst = ctxt.get_elem_jacobian(zc_var, aux_fconst_var);
	...
	DenseSubMatrix<Number> &J_fconst_auxfconst = ctxt.get_elem_jacobian(fconst_var, aux_fconst_var);
	DenseSubMatrix<Number> &J_fconst_auxzc = ctxt.get_elem_jacobian(fconst_var, aux_zc_var);
	...
	DenseSubMatrix<Number> &J_auxzc_fconst = ctxt.get_elem_jacobian(aux_zc_var, fconst_var);
	...
	DenseSubMatrix<Number> &J_auxfconst_fconst = ctxt.get_elem_jacobian(aux_fconst_var, fconst_var);
	DenseSubMatrix<Number> &J_auxfconst_zc = ctxt.get_elem_jacobian(aux_fconst_var, zc_var);
	...
	DenseSubVector<Number> &Rauxfconst = ctxt.get_elem_residual( aux_fconst_var );
	...
	for (unsigned int qp=0; qp != n_qpoints; qp++)
	  {
	    Number 
	      ...
	      auxzc = ctxt.interior_value(aux_zc_var, qp);
			Number fc, auxfc, fconst, auxfconst;
	    ...
	    NumberVectorValue U(u,v);
	      ...
	      if(subdomain == scalar_subdomain_id){
	      	Rauxzc(i) += JxW[qp]*(-k*grad_c*dphi[i][qp] - U*grad_c*phi[i][qp] + R*c*c*phi[i][qp] + fconst*phi[i][qp]);
	      	//Rauxfconst(i) += JxW[qp]*(beta*fc*phi[i][qp] + zc*phi[i][qp]); //DEBUG
	      }
	      ...
	      if((qoi_option == 1 && 
     		...
     		if(subdomain == scalar_subdomain_id){
	      	Rzc(i) += JxW[qp]*(-k*grad_auxc*dphi[i][qp] - U*grad_auxc*phi[i][qp] 
			   	...
			   	//Rfconst(i) += JxW[qp]*(auxzc*phi[i][qp] + beta*auxfc*phi[i][qp]); //DEBUG			
	      }
	      else if(subdomain == field_subdomain_id){
			    Rzc(i) += JxW[qp]*(-k*grad_auxc*dphi[i][qp] - U*grad_auxc*phi[i][qp] 
     				...
     				if((qoi_option == 1 && 
						...
						if(subdomain == field_subdomain_id){
					  	...
					  	J_fc_auxfc(i,j) += JxW[qp]*(beta*dphi[j][qp]*dphi[i][qp] + beta*phi[j][qp]*phi[i][qp]);
							...
							//J_zc_auxfconst(i,j) += JxW[qp]*(phi[j][qp]*phi[i][qp]); //DEBUG
							
							//J_fconst_auxzc(i,j) += JxW[qp]*(phi[j][qp])*phi[i][qp]; //DEBUG
					  	//J_fconst_auxfconst(i,j) += JxW[qp]*(beta*phi[j][qp]*phi[i][qp]); //DEBUG
		      	}
						...
						if(subdomain == field_subdomain_id){
					  	...
					  	J_auxfc_fc(i,j) += JxW[qp]*(beta*dphi[j][qp]*dphi[i][qp] + beta*phi[j][qp]*phi[i][qp]);
							...
							if(j == 0)
								J_auxzc_fconst(i,j) += JxW[qp]*phi[i][qp];
							//J_auxzc_fconst(i,j) += JxW[qp]*(phi[j][qp]*phi[i][qp]); //DEBUG
								
							//J_auxfconst_zc(i,j) += JxW[qp]*(phi[j][qp])*phi[i][qp]; //DEBUG
					  	//J_auxfconst_fconst(i,j) += JxW[qp]*(beta*phi[j][qp]*phi[i][qp]); //DEBUG
		      	}
