  ...
  for (unsigned int qp = 0; qp != n_qpoints; qp++)
    {
      ...
      NumberVectorValue U(u,v);
		...
		if(debug_step == 0){ //DEBUG
			//MHF_psiLF = I(q_LF, u_LF) + L'_HF(q_LF, u_LF, z_LF)(p_LF, v_LF, y_LF)
	    ...
	    Gradient 
	      ...
	      sadj_grad_auxfc = sadj_gradauxfc_stash[myElemID][qp];
   		...
   		if(subdomain == field_subdomain_id){
				...
				half_sadj_resid_elem += 
					JxW[qp]*(-k*grad_c*sadj_grad_auxzc - U*grad_c*sadj_auxzc + R*c*c*sadj_auxzc + const_vars[0]*sadj_auxzc);
				half_sadj_resid_elem += JxW[qp]*(beta*const_vars[0]*sadj_auxfc + zc*sadj_auxfc);
			}
      ...
      if((qoi_option == 1 && 
   		...
   		if(subdomain == field_subdomain_id){
		    half_sadj_resid_elem += JxW[qp]*(-k*grad_auxc*sadj_grad_zc - U*grad_auxc*sadj_zc 
   			...
   			half_sadj_resid_elem += JxW[qp]*(-k*grad_auxc*sadj_grad_zc - U*grad_auxc*sadj_zc 
		    						+ const_vars[1]*sadj_zc + 2*R*c*auxc*sadj_zc);
		 		half_sadj_resid_elem += JxW[qp]*(auxzc*sadj_fc + beta*const_vars[1]*sadj_fc); 
   		}
		}		
