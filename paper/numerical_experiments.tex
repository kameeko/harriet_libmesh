...
\begin{figure}[h]
\centering
\includegraphics[width=0.4\textwidth]{series3D/setup_arial_nolegend.pdf} \hfill
...
\end{tabular}
\caption{Runtime and relative errors of adaptive algorithm iterations.}
\label{tab:ref3D}
\end{table}
%
Compared to the previous examples in Section~\ref{sec:cdvcdr} with similar physics in the pair of models, we notice that in this case the estimated relative error in the later iterations is similar to the actual relative error in magnitude, but not in sign. This is likely due to the additional difference in the diffusive coefficient of the two models in this case. The sign difference does not, however, prevent the algorithm from finding a mixed-fidelity model for which the QoI matches the high-fidelity QoI to within $0.1\%$, but in about a third of the time. %what happens if you go on forever? (running)

The domain divisions for the three adaptive iterations are shown in Figure~\ref{fig:divvy3D}. We see that the error decomposition causes the first refinement iteration to target the QoI region and observations. %is there a way to see if any observations are more 'important' than others?
%
\begin{figure}[h]
\centering
\begin{subfigure}[t]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth]{series3D/divvy1_whitebg_shiftaxes.png} 
\caption{MF$_1$}
\end{subfigure}
\begin{subfigure}[t]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth]{series3D/divvy2_whitebg_shiftaxes.png} 
\caption{MF$_2$}
\end{subfigure}
\begin{subfigure}[t]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth]{series3D/divvy3_whitebg_shiftaxes.png} 
\caption{MF$_3$}
\end{subfigure}
\caption{Domain division for mixed-fidelity models (low-fidelity convection-diffusion model used in red portion, high-fidelity convection-diffusion-reaction model used in blue portion).}
\label{fig:divvy3D}
\end{figure} 
%
The amount of time required by the adaptive algorithm of course depends on the chosen error tolerance as well as the amount of additional model refinement between successive iterations. We also compare the runtimes for a more conservative choice of refinement rate; given the same error tolerance, we instead choose at each iteration the basis functions with the largest $5\%$ of the error for model refinement. Table~\ref{tab:ref3D_dainty} shows the runtime and error at the end of each of these iterations.
%
\begin{table}[h]
\centering
\begin{tabular}{c|c|c|c|c|c}
...
\end{table}
%
%similar final refinement percentage
%can we get a better sense of 'important' observations?
%same weird sign flip at end
%actual relative error decreases monotonically in magnitude, but not estimated...
