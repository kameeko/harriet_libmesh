...
%
We now use Algorithm~\ref{alg:refSeries} to solve goal oriented inverse problems in a multi-model setting. We consider two kinds of multi-fidelity models, in a total of three experiments. In the first experiment, the high fidelity model is a convection-diffusion-reaction nonlinear model, and the low fidelity model is a linear convection-diffusion model. We examine how the element-wise distribution of the error estimate is affected by changes in sensor placement and in the QoI region. In the second experiment, the high fidelity model uses an infinite dimensional (field) representation of the inferred parameter, while the low fidelity model uses a scalar representation. \red{The third experiment has the same setting as the first one, except a highly nonlinear version of the reaction term is considered, and showcases the robustness and computatioanal cost benefits of using the adaptive algorithm.}
...
Figure~\ref{fig:baseRef} shows the element-wise decomposition of the error estimate \red{no longer constant-per-element, what exactly does this plot show?}, as well as the subdomains where the low- and high-fidelity models are used, for the series of mixed-fidelity models thus generated. \red{note that plots show absolute values, so valleys between lobes are actually transition between positive and negative...also note which decomp method used if comparing...}
%
\begin{figure}[h!]
\centering
...
Figure~\ref{fig:baseErr} shows the true and estimated absolute relative errors in the QoI for the various mixed-fidelity models generated by Algorithm~\ref{alg:refSeries}. \red{It can be seen that the error estimates are close to the true error, and the error estimator has an effectivity index close to one.} We also see that the QoI error of only $1\%$ is attained with a mixed-fidelity model where the high-fidelity model is used in only about $10\%$ of the domain.
%
\begin{figure}[h]
\centering
...
\red{Does it make sense to compare the breakdowns and error vs refinement of the different error localization schemes? Or is one so obviously better than the other that we don't need to show it?}
