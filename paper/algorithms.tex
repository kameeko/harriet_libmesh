...
\Procedure{$\texttt{BuildMF}$}{HF model, LF model, \texttt{errTol}, \texttt{maxIter}}
	...
	\While{$i<$ \texttt{maxIter} and $|\epsilon_i/I(q_{MF_i},u_{MF_i})|>$ \texttt{errTol}}
		\State{\begin{varwidth}[t]{\linewidth}Localize $\epsilon_i$ (see section~\ref{sect:error_est_localize}) and use this decomposition to guide formation of new \par\hskip\algorithmicindent mixed-fidelity model MF$_{i+1}$\end{varwidth}}
		\State{$i\gets i+1$}
