...
\Procedure{$\texttt{BuildMF}$}{HF model, LF model, \texttt{errTol}, \texttt{maxIter}}
	...
	supplementary adjoint $\Lambda_0$ (see \cref{eq:superAdjEq})}
	...
	\While{$i<$ \texttt{maxIter} and $|\epsilon_i/I(q_{MF_i},u_{MF_i})|>$ \texttt{errTol}}
		\State{\begin{varwidth}[t]{\linewidth}Localize $\epsilon_i$ and use this decomposition to guide formation of new \par\hskip\algorithmicindent mixed-fidelity model MF$_{i+1}$\end{varwidth}}
		\State{$i\gets i+1$}
...
Comparing \cref{eq:cost_adapt} and \cref{eq:cost_nadapt}, we have,
%
\begin{equation}
