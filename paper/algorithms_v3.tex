...
\Procedure{$\texttt{BuildMixedFidelity}$}{HF model, LF model, \texttt{errTol}, \texttt{maxIter}}
	...
	\State{Solve QoI error adjoint equation
	%, linearized about $\Psi_{MF_0}$,
	 for \par\hskip\algorithmicindent supplementary adjoint $\Lambda_0$ (see \cref{eq:superAdjEq})}
	...
	\While{$i<$ \texttt{maxIter} and $|\epsilon_i/(\epsilon_i+I(q_{MF_i},u_{MF_i}))|>$ \texttt{errTol}}
		...
		\State{Solve QoI error adjoint equation%, linearized about $\Psi_{MF_i}$,
		 for
