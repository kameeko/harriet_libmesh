...
\cref{fig:baseSetup} shows the locations of the observations and the region $\Omega_I$ over which the QoI is calculated. For the low-fidelity model, the inverse problem is linear (i.e., the observations depend linearly on the parameters). Since the inverse problem is ill-posed, we use Tikhonov regularization~\cite{EngHanNeu00}; the regularization term is $\frac{\beta}{2}\int_\Omega \|\nabla f(q)\|_2^2\:\textrm{d}A$, where $\beta=10^{-5}$ is the regularization coefficient. % KW: term in what equation? give a reference or re-state the objective here.
%
\begin{figure}[htbp]
\centering
...
We see that the largest local error contribution is concentrated in the QoI region, and the data point closest to the QoI. 
% KW: I am not sure how to read the second phrase in this first sentence. Is it '...contribution is concentrated in two places: in the QoI region and near the observation location closest to the QoI.'  ??
In the first decomposition of the error (\cref{fig:baseRef0}), the region where the elemental error is maximum is the leftmost data point. % KW: again here, what is a 'data point' ? Do you mean 'observation location' ?
Since the constraining model is an elliptic PDE, with weak convection, information flow is localized, and is weakly convected from left to right. Therefore, for the calculation of the QoI, it is most important to refine the region near the leftmost data point, and the QoI region. % KW: same here. plus another confusing comma.
After that, the error decomposition suggests refinement in regions upstream and around the middle data point, and then the rightmost data point. % KW: and here
...
The error decompositions for each case are also shown in \cref{fig:qoiStudy}. The bottom row gives the baseline case presented in \cref{sec:cdvcdrBaseRef}, although here we choose the basis functions $i$ whose error $\varepsilon_i$ are among the largest $5\%$, so the proportion of additional refined elements in each iteration is slightly larger.
...
% KW: fix references to 'data point' throughout.
...
\subfloat[MF$_2$ ($\sim10\%$ HF)][MF$_2$ \\($\sim10\%$ HF)]{
  ...
  \caption{Effects of increasing the QoI region. Column \protect\subref{subfig:obsSetup2}: configuration of observations (teal points) and QoI region (purple box). Columns \protect\subref{subfig:obsLF2}--\protect\subref{subfig:obsMFlast2}): the error estimate decompositions for different mixed-fidelity models.}
  \label{fig:qoiStudy}
\end{figure}
...
in the case where the low-fidelity model is used throughout the domain, regularization is added; % KW: added to what?
 the Tikhonov regularization term is $\frac{\beta}{2}\int_\Omega \|\nabla f(q)\|_2^2+f(q)^2\:\textrm{d}A$, where $\beta=10^{-3}$ is a regularization coefficient. % KW: refer to the actual equation you are adding regularization to, or re-state the objective.

% KW: I'm commenting out this next statement, unless you strongly disagree.
...
\Cref{fig:svfRef} shows the local error contributions, as well as the subdomains where the low- and high-fidelity models are used, for the first two and last mixed-fidelity model thus generated. Each linear Lagrange basis function's contribution is plotted at its nonzero node. % KW: what does this sentence mean and is it really necessary?
%
\begin{figure}[htbp]
\centering
\subfloat[MF$_0$ ($0\%$ HF)]{
...
The unknown parameters we wish to infer correspond to the source term $f(q)=q$; we impose $f(q)=q=0$ on the boundary $\partial\Omega$. Synthetic observations at 18 points in the domain are generated by running the high-fidelity model on a finer mesh. The locations of the observations as well as the QoI region $\Omega_I$ are shown in \cref{fig:setup3D}. We use a regularization term $\frac{\beta}{2}\int_\Omega \|\nabla f(q)\|_2^2\:\textrm{d}V$. % KW: again please connect reg term to an equation or just state objective function here.
%
\begin{figure}[htbp]
\centering
...
% KW: this paragraph is a bit rambling. I think it is obvious: you do better when you optimize the model for the actual dataset. I think we leave this out unless you think there is a critical point here that is not captured elsewhere.
