...
%DIF DEL paper_v4_cmame.tex          Sat Jul  1 07:37:11 2017
%DIF ADD paper_v4_cmame_vikram.tex   Tue Jun 20 15:59:15 2017
%%\usepackage{lgrind}
...
%DIF 48a48
\newcommand{\blue}[1]{{\color{blue}{#1}}} %DIF > 
%DIF -------
...
The choice of model should be informed by one's goal: for a range of applications, the parameters may be numerous, such as when corresponding to a discretized field, and yet what is ultimately of interest may be some low-dimensional quantity of interest (QoI). We consider this \textit{goal-oriented inverse problem}, where the goal of inferring parameters is to use them in predicting a QoI. In this context, one may choose a lower-fidelity model for the inference problem, sacrificing accuracy in the state and/or parameter estimates in exchange for reduced computational costs while keeping the error in the QoI to within some acceptable tolerance. \DIFaddbegin \DIFadd{Compared to the highest-fidelity model, the lower-fidelity model could, for example, be a simplified model including fewer physical phenomena, be a reduced-order model, or have a reduced parameter space; this generalizes the setup where one only refines the mesh to create high-fidelity models. }\DIFaddend This paper formally poses the problem of managing the fidelity of modeling choices in solving the goal-oriented inverse problem, and develops an adjoint-based model adaptivity approach that achieves a desired level of accuracy in the QoI prediction. In particular, we adaptively form a mixed-fidelity model by using models of different levels of fidelity in different parts of the domain. %DIF > Compared to the high-fidelity model, inclusion of low-fidelity regions can make this mixed-fidelity model less nonlinear, and/or have a reduced state and/or parameter space;. 
...
Goal-oriented approaches prioritize accuracy in the QoI over accuracy in the states and/or parameters; one aims to reduce the cost of solving the problem (whether forward or inverse) while maintaining an acceptably low error tolerance for the QoI. In the context of the forward problem, goal-oriented methods have been developed for mesh refinement \cite{BecRann01,Park04,PrudOden99,VendDarm00,Yano12} and for \DIFaddbegin \DIFadd{more general }\DIFaddend model adaptivity~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{BraackErn03,OdenPrudetal06}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{OdenVem00,VemOden01,BraackErn03,OdenPrudetal06}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . These methods derive an adjoint-based a posteriori error estimate for an output functional, and this estimate is used to guide adaptive mesh and/or model refinement. \DIFaddbegin \DIFadd{Approaches for the goal-oriented forward problem can be applied to the inverse problem by treating the optimality conditions of the inverse problem as a forward model, though one more complex than the original forward model. Methods tailored to the goal-oriented inverse problem, including the one proposed in this work, make use of the particular structure of the inverse problem. 
}\DIFaddend 
...
%
\DIFdelbegin \DIFdel{We first introduce the goal-oriented inverse problem. Then we derive a rigorous a posteriori error estimate for the error induced }\DIFdelend \DIFaddbegin \DIFadd{In this section, we introduce the high-, low- and mixed-fidelity inverse problems. We develop theoretical results which extend the work in~\mbox{%DIFAUXCMD
\cite{BecVex05} }\hspace{0pt}%DIFAUXCMD
to multi-model settings; in particular, we obtain a computable error estimate }\DIFaddend in the QoI \DIFdelbegin \DIFdel{due to the use }\DIFdelend \DIFaddbegin \DIFadd{that can be used for targeted generation }\DIFaddend of mixed-fidelity \DIFdelbegin \DIFdel{and/or lower-fidelity models}\DIFdelend \DIFaddbegin \DIFadd{formulations of inverse problems}\DIFaddend .

%------------------------------------------------------------------------------%
\subsection{Inverse Problem Formulation}  \label{sec:setup}
%------------------------------------------------------------------------------%
%
Consider \DIFdelbegin \DIFdel{a model for which the Galerkin formulation of the weak form is written as
}\begin{displaymath}
\DIFdel{a(u,q)(\phi)=\ell(q)(\phi),\quad\forall\phi\in U,
%DIFDELCMD < \label{eq:weakForm}%%%
}\end{displaymath}
%DIFAUXCMD
\DIFdel{where$u\in U$ is the state, }\DIFdelend \DIFaddbegin \DIFadd{the inverse problem where, given observations (data) $d\in\R^{n_d}$, we seek to infer parameter(s) }\DIFaddend $q\in Q$\DIFdelbegin \DIFdel{are the unknown parameters, $\phi$ is a test function }\DIFdelend , \DIFdelbegin \DIFdel{and the state space $U$ and parameter space }\DIFdelend \DIFaddbegin \DIFadd{where $Q$ is some function space. In this exposition, we assume that }\DIFaddend $Q$ \DIFdelbegin \DIFdel{are Hilbert spaces. }\DIFdelend \DIFaddbegin \DIFadd{is a Hilbert space. An observation operator $C:U\to\Reals^{n_d}$ relates the parameter to the observations via state variable(s) $u\in U$, with $u$ satisfying,
%DIF > 
}\begin{equation}
\DIFadd{a(u,q)(\phi)=\ell(q)(\phi),\quad\forall\phi\in U,
\label{eq:weakForm}
}\end{equation}
%DIF > 
\DIFadd{where $U$ is also a Hilbert space. Equation~\ref{eq:weakForm} is called the state equation. }\DIFaddend The form $a$\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, and the }\DIFaddend functional $\ell$ are linear with respect to the arguments in the second pair of parentheses (in \Cref{eq:weakForm}, they are linear with respect to $\phi$).
\DIFdelbegin \DIFdel{Further, we define an observation operator $C:U\to\Reals^{n_d}$ that maps the state to $n_d$ predicted observations. The actual observations (data) are denoted by $d\in\R^{n_d}$.
}\DIFdelend 

The unknown parameter $q$ can be inferred by minimizing the difference between the predicted and actual observations \DIFaddbegin \DIFadd{in a chosen norm}\DIFaddend , leading to an inverse problem. \DIFdelbegin \DIFdel{In our setting, such inverse problems are typically }\DIFdelend \DIFaddbegin \DIFadd{We consider the case where observations are sparse; this leads to inverse problems that are }\DIFaddend ill-posed, since the \DIFdelbegin \DIFdel{observations are sparse, and }\DIFdelend \DIFaddbegin \DIFadd{sparse observations are }\DIFaddend insufficiently informative to uniquely determine the parameters. To make the inverse problem well-posed, a regularization, denoted by $R(q)$, is used to inject prior information or beliefs about the parameters into the formulation. The regularized inverse problem can thus be written as a constrained optimization problem,
%
\begin{subequations}
\label{eq:invOpt}
\begin{align}
...
\DIFdelbegin \DIFdel{The constraints \mbox{%DIFAUXCMD
\Cref{eq:invOpt_cons} }\hspace{0pt}%DIFAUXCMD
are typically models of physical processes or systems. }\DIFdelend \DIFaddbegin 

\DIFaddend A given physical system can be described to varying degrees of fidelity using different models. \DIFdelbegin \DIFdel{This naturally introduces a hierarchy of models and a tradeoff between fidelity and computational expense; how one manages this tradeoff depends on }\DIFdelend \DIFaddbegin \DIFadd{One has a choice in how to define the stucture of the inferred parameters $q$, as well as how to define the state equation and variables. For example, the parameter $q$ can be modeled as a single scalar value or as an infinite-dimensional distributed field variable. The state equation can be nonlinear and multiscale, requiring an expensive computation to resolve (e.g.\ the Navier-Stokes equations) or a linear, single physics model (e.g.\ Laplace's equation). All these different modeling choices lead to different versions of the optimization problem described by \mbox{%DIFAUXCMD
\Cref{eq:invOpt} }\hspace{0pt}%DIFAUXCMD
that would need to be solved to infer the unknown parameters.
}

\DIFadd{We assume that there exists an `ideal' model for solving the inverse problem, which we will call the high-fidelity model; given adequate computational resources, we would use this high-fidelity to formulate and solve our inverse problem. We use the subscript $_{HF}$ to denote the high-fidelity inverse problem,
%DIF > 
}\begin{subequations}
\label{eq:invOptHF}
\begin{align}
\DIFadd{q_{HF},u_{HF} }&\DIFadd{=}&\DIFadd{\arg\min\limits_{q,u} }& \DIFadd{\quad J(q,u)=\frac{1}{2}\|d-C(u)\|_2^2 + R(q), \label{eq:invOpt_objHF} }\\
&&\DIFadd{\textrm{s.t. }}& \DIFadd{\quad a_{HF}(u,q)(\phi)=\ell_{HF}(q)(\phi),\quad\forall\phi\in U_{HF} \label{eq:invOpt_consHF}
}\end{align}
\end{subequations}
%DIF > 
\DIFadd{where $q_{HF} \in Q_{HF}$ and $u_{HF}\in U_{HF}$ are the inferred parameters and corresponding state. In practice, one has limited computational resources, so one may need to instead infer the parameters using a less computationally demanding model, which we call the low-fidelity model. We use the subscript $_{LF}$ to denote the low-fidelity inverse problem,
%DIF > 
}\begin{subequations}
\label{eq:invOptLF}
\begin{align}
\DIFadd{q_{LF},u_{LF} }&\DIFadd{=}& \DIFadd{\arg\min\limits_{q,u} }& \DIFadd{\quad J(q,u)=\frac{1}{2}\|d-C(u)\|_2^2 + R(q), \label{eq:invOpt_objLF} }\\
&& \DIFadd{\textrm{s.t. }}& \DIFadd{\quad a_{LF}(u,q)(\phi)=\ell_{LF}(q)(\phi),\quad\forall\phi\in U_{LF}, \label{eq:invOpt_consLF}
}\end{align}
\end{subequations}
%DIF > 
\DIFadd{where $q_{LF} \in Q_{LF}$ and $u_{LF}\in U_{LF}$ are the inferred parameters and corresponding state. If there is compatibility between the low-fidelity and high-fidelity solution spaces for the parameter and state variables (the solutions of one can be interpreted in the context of the other; see \mbox{%DIFAUXCMD
\Cref{sec:adapt_alg} }\hspace{0pt}%DIFAUXCMD
for more details), one may combine the two models to form a mixed-fidelity model and corresponding inverse problem. The low-fidelity model can be used in part of the computational domain, and the high fidelity in the remainder; we denote the mixed-fidelity model and its variables with the subscript $_{MF}$. How one chooses to combine the low- and high-fidelity models can be informed by }\DIFaddend what aspect of the \DIFdelbegin \DIFdel{solution }\DIFdelend \DIFaddbegin \DIFadd{inferred parameters }\DIFaddend is of interest.

\DIFdelbegin \DIFdel{We consider the case of }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend a {\em goal-oriented inverse problem}, \DIFdelbegin \DIFdel{where }\DIFdelend the ultimate purpose of inferring the unknown parameters is to calculate some quantity of interest (QoI). We denote a scalar QoI by \DIFdelbegin \DIFdel{$I(q,u)$, where $I:Q\times U\to\R$ is }\DIFdelend a functional that maps the parameters and state to our QoI\DIFdelbegin \DIFdel{. We then }\DIFdelend \DIFaddbegin \DIFadd{, $I:Q \times U \to \R$. The QoIs evaluated with the high- ($I(q_{HF},u_{HF})$), low- ($I(q_{LF},u_{LF})$), or mixed- ($I(q_{MF},u_{MF})$) fidelity models correspondingly map parameters and state from the high-, low- or mixed- fidelity function spaces to QoI values. We }\DIFaddend consider the tradeoff between the error in the QoI and the fidelity (and corresponding computational expense) of the model we use\DIFdelbegin \DIFdel{. In the following, we }\DIFdelend \DIFaddbegin \DIFadd{, seeking computable estimates for the error incurred in approximating the high-fidelity model with lower-fidelity models. Such an estimate allows us to adaptively choose regions where the high-fidelity model is used, starting from an initial low- or mixed- fidelity model.
}

%DIF > We build on the work in~\cite{BecVex05, becker2004posteriori} which utilize the special structure of the coupled system of PDEs induced by deriving optimality conditions for~\Cref{eq:invOpt_cons}, and derives error estimates for the discretization error in a single model setting. Details of why such an approach is effective for the nature of systems and QoIs we encounter in inverse problems can be found in~\cite{becker2004posteriori} (see Section 3.1 in particular). Here we extend the work in~\cite{BecVex05} to include the situation where the model structure differs, and it is the modeling error that needs to be estimated.

\DIFadd{We build on the work in~\mbox{%DIFAUXCMD
\cite{BecVex05, becker2004posteriori} }\hspace{0pt}%DIFAUXCMD
which utilize the special structure of the coupled system of PDEs corresponding to the optimality conditions for~\mbox{%DIFAUXCMD
\Cref{eq:invOpt} }\hspace{0pt}%DIFAUXCMD
to derive estimates for the QoI error due to discretization in a single model setting; they consider the case where the high- and low-fidelity models represent different discretizations in the variables corresponding to the same infinite-dimensional model. Details of how the structure of the goal-oriented inverse problem is exploited can be found in~\mbox{%DIFAUXCMD
\cite{becker2004posteriori} }\hspace{0pt}%DIFAUXCMD
(see Section 3.1 in particular). Here we extend the work in~\mbox{%DIFAUXCMD
\cite{BecVex05} }\hspace{0pt}%DIFAUXCMD
to include the situation where state equations the high- and low-fidelity models differ in more than the discretization of the variable spaces, and it is the QoI error induced by modeling differences that needs to be estimated.
}

\DIFadd{As in~\mbox{%DIFAUXCMD
\cite{BecVex05}}\hspace{0pt}%DIFAUXCMD
, we }\DIFaddend introduce the QoI functional $I$ into the inverse problem formulation by introducing auxiliary variables and additional adjoint equations. We then use this formulation to derive an a posteriori error estimate for \DIFdelbegin \DIFdel{$I$}\DIFdelend \DIFaddbegin \DIFadd{the QoI}\DIFaddend , where the errors considered are those due to the use of different multi-fidelity models\DIFdelbegin \DIFdel{in the constraint \mbox{%DIFAUXCMD
\Cref{eq:invOpt_cons}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{. However, unlike in~\mbox{%DIFAUXCMD
\cite{BecVex05}}\hspace{0pt}%DIFAUXCMD
, this estimate cannot be computed by patch recovery or projection techniques, since the low- and high- fidelity solutions do not differ merely in mesh resolution, but in the underlying equations being discretized. To compute this error estimate, we instead introduce additional supplementary adjoint variables; these new variables serve a similar role to the patch recovery or projection techniques in that they carry information about the high-fidelity solution to the error estimate.
}
...
%------------------------------------------------------------------------------%
%
For a given hierarchy of models, \DIFdelbegin \DIFdel{consider }\DIFdelend \DIFaddbegin \DIFadd{we take }\DIFaddend the QoI calculated from inferring the parameters with the highest-fidelity model \DIFdelbegin \DIFdel{; we take this QoI }\DIFdelend to be the \DIFdelbegin \DIFdel{value with which we compare other QoI estimates. In this section we }\DIFdelend \DIFaddbegin \DIFadd{`exact' or `true' solution; we now }\DIFaddend derive an a posteriori estimate for the error in the QoI from inferring the parameters with a lower-fidelity model.
%
\begin{proposition}
\label{thm:error_estimate}
Consider the \DIFdelbegin \DIFdel{inverse problem }\DIFdelend \DIFaddbegin \DIFadd{high- and low-fidelity inverse problems }\DIFaddend described by the constrained optimization \DIFdelbegin \DIFdel{problem \mbox{%DIFAUXCMD
\Cref{eq:invOpt}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{problems in \mbox{%DIFAUXCMD
\Cref{eq:invOptHF,eq:invOptLF}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Let the \DIFdelbegin \DIFdel{form $a:U \times U \to \Reals$ }\DIFdelend \DIFaddbegin \DIFadd{forms $a_{HF/LF}:U_{HF/LF} \times Q_{HF/LF} \times U_{HF/LF} \to \Reals$ }\DIFaddend be three times continuously differentiable with respect to the state $u$ and parameters $q$. Let the observation operator \DIFdelbegin \DIFdel{$C:U\to\Reals^{n_d}$ }\DIFdelend \DIFaddbegin \DIFadd{$C:U_{HF/LF}\to\Reals^{n_d}$ }\DIFaddend be three times continuously differentiable with respect to the state $u$. Also, let the regularization operator \DIFdelbegin \DIFdel{$R:Q\to\Reals$ }\DIFdelend \DIFaddbegin \DIFadd{$R:Q_{HF/LF}\to\Reals$ }\DIFaddend be differentiable with respect to the parameter $q$, and the \DIFdelbegin \DIFdel{functional $I:Q\times U\to\Reals$ }\DIFdelend \DIFaddbegin \DIFadd{Quantity of Interest (QoI) functional $I:Q_{HF/LF}\times U_{HF/LF}\to\Reals$ }\DIFaddend be differentiable with respect to the state $u$ and parameter $q$.

Consider the Lagrangian equation induced by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\Cref{eq:invOpt}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\Cref{eq:invOptHF}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ,
%
\begin{equation}
\label{eq:InvsOpt_lag}
\mathcal{L}\DIFaddbegin \DIFadd{_{HF}}\DIFaddend (q,u,z)= J\DIFaddbegin \DIFadd{_{HF}}\DIFaddend (q,u)-(a\DIFaddbegin \DIFadd{_{HF}}\DIFaddend (u,q)(z)-\ell\DIFaddbegin \DIFadd{_{HF}}\DIFaddend (q)(z)),
\end{equation}
%
where \DIFdelbegin \DIFdel{$z\in U$ }\DIFdelend \DIFaddbegin \DIFadd{$z\in U_{HF}$ }\DIFaddend is the adjoint. Denoting the primary variables as $\xi=(q,u,z)$, introduce corresponding auxiliary variables \DIFdelbegin \DIFdel{$\chi=(p,v,y)\in Q\times U\times U$}\DIFdelend \DIFaddbegin \DIFadd{$\chi=(p,v,y)\in Q_{HF}\times U_{HF}\times U_{HF}$}\DIFaddend . Let the augmented Lagrangian be defined as
%
\begin{equation}
\label{eq:InvsOpt_auglag}
\mathcal{M}\DIFaddbegin \DIFadd{_{HF}}\DIFaddend ((q,u,z),(p,v,y)) = I(q,u) + \mathcal{L}\DIFdelbegin \DIFdel{_{quz}}\DIFdelend '\DIFaddbegin \DIFadd{_{HF}}\DIFaddend (q,u,z)(p,v,y),
\end{equation}
%
where \DIFdelbegin \DIFdel{$\mathcal{L}_{quz}'(q,u,z)(p,v,y)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{L}'_{HF}(q,u,z)(p,v,y)$ }\DIFaddend denotes the Fr\'{e}chet derivative of the Lagrangian about the primary variables $(q,u,z)$, in the direction of the auxiliary variables $(p,v,y)$. \DIFdelbegin \DIFdel{Let $\Psi = (\xi_\Psi,\chi_\Psi)$ denote the stationary point of $\mathcal{M}$.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Consider two models with which parameters can be inferred: a high-fidelity model and a lower-fidelity model. Let the high-fidelity (HF) and }\DIFdelend \DIFaddbegin \DIFadd{One can define a similar Langrangian $\mathcal{L}_{LF}$ induced by \mbox{%DIFAUXCMD
\Cref{eq:invOptLF}}\hspace{0pt}%DIFAUXCMD
, and a corresponding augmented Lagrangian $\mathcal{M}_{LF}$, for the }\DIFaddend low-fidelity \DIFdelbegin \DIFdel{(LF) models, and their corresponding variables, Lagrangians, and augmented Lagrangians, be distinguished by the subscripts $_{HF}$ and $_{LF}$, respectively. In particular, let $\Psi_{HF}$ and $\Psi_{LF}$ }\DIFdelend \DIFaddbegin \DIFadd{model. Let $\Psi_{HF}= (\xi_{HF},\chi_{HF})=((q_{HF},u_{HF},z_{HF}),(p_{HF},v_{HF},y_{HF}))$ and $\Psi_{LF}= (\xi_{LF},\chi_{LF})=((q_{LF},u_{LF},z_{LF}),(p_{LF},v_{LF},y_{LF}))$ }\DIFaddend denote the stationary points of the high- and low- fidelity augmented Lagrangians\DIFdelbegin \DIFdel{$\mathcal{M}_{HF}$ and $\mathcal{M}_{LF}$}\DIFdelend , respectively. Then, the error in the \DIFdelbegin \DIFdel{Quantity of Interest $I$ }\DIFdelend \DIFaddbegin \DIFadd{QoI }\DIFaddend is given by\DIFaddbegin \DIFadd{,
}\DIFaddend %
\begin{multline}
\label{eq:semifinErrExp}
I(q_{HF},u_{HF})-I(q_{LF},u_{LF})=\\\frac{1}{2}\mathcal{M}'\DIFdelbegin \DIFdel{_{HF,\Psi}}\DIFdelend \DIFaddbegin \DIFadd{_{HF}}\DIFaddend (\Psi_{LF})(\Psi_{HF}-\Psi_{LF})+\mathcal{M}_{HF}(\Psi_{LF})-\mathcal{M}_{LF}(\Psi_{LF})+\mathcal{R}\DIFdelbegin \DIFdel{(e^3)}\DIFdelend \textrm{,}
\end{multline}
%
...
\begin{equation}
\label{eq:MeqI}
\mathcal{M}\DIFaddbegin \DIFadd{_{HF}}\DIFaddend (\Psi\DIFaddbegin \DIFadd{_{HF}}\DIFaddend )=I(q\DIFaddbegin \DIFadd{_{HF}}\DIFaddend ,u\DIFaddbegin \DIFadd{_{HF}}\DIFaddend )\DIFaddbegin \quad\textrm{\DIFadd{and}}\quad\DIFadd{\mathcal{M}_{LF}(\Psi_{LF})=I(q_{LF}}\DIFaddend ,\DIFaddbegin \DIFadd{u_{LF}),
}\DIFaddend \end{equation}
%
since taking variations of \DIFdelbegin \DIFdel{$\mathcal{M}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{M}_{HF}$ and $\mathcal{M}_{LF}$ }\DIFaddend with respect to the auxiliary variables gives that \DIFdelbegin \DIFdel{$\xi_\Psi$ is a stationary point of $\mathcal{L}$}\DIFdelend \DIFaddbegin \DIFadd{$\xi_{HF}$ and $\xi_{LF}$ are stationary points of $\mathcal{L}_{HF}$ and $\mathcal{L}_{LF}$, respectively}\DIFaddend .

Extending the property in \Cref{eq:MeqI} to the augmented Lagrangians for the high- and low-fidelity models, we have\DIFaddbegin \DIFadd{,
}\DIFaddend %
\begin{multline}
\label{eq:repIwithM}
...
\end{multline}
%
\DIFdelbegin \DIFdel{Applying the output error representation described inProposition 3 from~\mbox{%DIFAUXCMD
\cite{BecVex05} }\hspace{0pt}%DIFAUXCMD
for the difference $\mathcal{M}_{HF}(\Psi_{HF})-\mathcal{M}_{HF}(\Psi_{LF})$,
}\DIFdelend \DIFaddbegin \DIFadd{Following~\mbox{%DIFAUXCMD
\cite{BecVex05}}\hspace{0pt}%DIFAUXCMD
, we rewrite the term $\mathcal{M}_{HF}(\Psi_{HF})-\mathcal{M}_{HF}(\Psi_{LF})$ using the Fundamental Theorem of Calculus,
%DIF > 
}\begin{equation}
\DIFadd{\label{eq:Mantiderivative}
...
%DIF > 
\DIFadd{Applying the trapezoidal rule to the integral in~\mbox{%DIFAUXCMD
\Cref{eq:Mantiderivative} }\hspace{0pt}%DIFAUXCMD
gives,
%DIF > 
}\begin{equation}
\DIFadd{\label{eq:Mprimetrapezoid}
...
%DIF > 
\DIFadd{where the remainder $\mathcal{R}$ is given as,
%DIF > 
}\DIFaddend \begin{equation}
\DIFdelbegin %DIFDELCMD < \label{eq:beckvex}
%DIFDELCMD < %%%
\DIFdel{\mathcal{M}_{HF}(\Psi_{HF})-\mathcal{M}_{HF}(\Psi_{LF}) }\DIFdelend \DIFaddbegin \label{eq:trapezoidremainder}
\DIFadd{\mathcal{R} = \frac{1}{2} \int\limits_{0}^{1} \mathcal{M}'''_{HF}\left(\Psi_{LF} + se\right)(e,e,e) \, s(s-1) \, \textrm{d}s,}\DIFaddend \end{equation}
%DIF > 
\DIFaddbegin \DIFadd{which is finite due to the third-order differentiability assumptions. We further note that $\mathcal{M}'_{HF}(\Psi_{HF})(e)$ vanishes since $\Psi_{HF}$ is a stationary point of $\mathcal{M}_{HF}$, and~\mbox{%DIFAUXCMD
\Cref{eq:Mprimetrapezoid} }\hspace{0pt}%DIFAUXCMD
can be simplified to,
%DIF > 
}\begin{equation}
\DIFadd{\label{eq:beckvex}
\mathcal{M}_{HF}(\Psi_{HF})-\mathcal{M}_{HF}(\Psi_{LF}) = \frac{1}{2}\mathcal{M}'_{HF}(\Psi_{LF})(\Psi_{HF}-\Psi_{LF})+\mathcal{R}\textrm{.}
}\end{equation}
%DIF > 
\DIFaddend Combining \Cref{eq:repIwithM} and \Cref{eq:beckvex} we obtain\DIFaddbegin \DIFadd{,
%DIF > 
...
The above \DIFdelbegin \DIFdel{proposition }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\Cref{thm:error_estimate} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend splits the error in the QoI into three components\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend a third-order remainder term \DIFdelbegin \DIFdel{$\mathcal{R}(e^3)$}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{R}$}\DIFaddend , a computable `bias' term $\mathcal{M}_{HF}(\Psi_{LF})-\mathcal{M}_{LF}(\Psi_{LF})$, and a linear term \DIFdelbegin \DIFdel{$\frac{1}{2}\mathcal{M}'_{HF,\Psi}(\Psi_{LF})(\Psi_{HF}-\Psi_{LF})$}\DIFdelend \DIFaddbegin \DIFadd{$\frac{1}{2}\mathcal{M}'_{HF}(\Psi_{LF})(\Psi_{HF}-\Psi_{LF})$}\DIFaddend . Although the linear term is not computable \DIFaddbegin \DIFadd{without solving for $\Psi_{HF}$}\DIFaddend , it can be estimated\DIFdelbegin \DIFdel{using an appropriate dual problem}\DIFdelend . We describe one method of estimating this term\DIFaddbegin \DIFadd{, using an appropriate dual problem, in \mbox{%DIFAUXCMD
\Cref{thm:error_estimate_dual} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend below.

\DIFdelbegin \DIFdel{Note that, as a stationary point, $\Psi_{HF}$ satisfies~\mbox{%DIFAUXCMD
\Cref{eq:supadjsys}}\hspace{0pt}%DIFAUXCMD
. Now, let $\mathcal{Q}$ be an output defined by
}\DIFdelend %
\DIFdelbegin \begin{displaymath}
\DIFdel{\mathcal{Q}(\Phi)=\mathcal{M}'_{HF,\Psi}(\Psi_{LF})(\Phi).
%DIFDELCMD < \label{eq:supadjout}%%%
}\end{displaymath}
%DIFAUXCMD
%DIF < 
\DIFdelend \DIFaddbegin \begin{proposition}
\label{thm:error_estimate_dual}
\DIFadd{Let the hypotheses of Proposition~\ref{thm:error_estimate} hold. }\DIFaddend Let the variational form
%
\begin{equation}
\mathscr{R}(\Psi_{HF})(\Phi)=0,\quad\forall\Phi\in(Q_{HF}\times U_{HF}\times U_{HF})^2
\DIFdelbegin %DIFDELCMD < \label{eq:supadjsys}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq:supadjsys_prop}
\DIFaddend \end{equation}
%
represent the equation $\mathcal{M}'_{HF,\Psi}(\Psi_{HF})(\Phi)=0$ in residual form, where $\Phi$ is a test function. Define \DIFdelbegin \DIFdel{the corresponding }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend adjoint problem
%
\begin{equation}
\mathscr{R}\DIFdelbegin \DIFdel{_{\Psi}}\DIFdelend '(\Psi\DIFdelbegin \DIFdel{_{HF}}\DIFdelend \DIFaddbegin \DIFadd{_{LF}}\DIFaddend ,\Phi)(\Lambda)=\mathcal{Q}(\Phi),\quad\forall\Phi\in(Q_{HF}\times U_{HF}\times U_{HF})^2
\label{eq:superAdjEq}
\end{equation}
for the supplementary adjoint $\Lambda$, where $\mathscr{R}$ is defined in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\Cref{eq:supadjsys}}\hspace{0pt}%DIFAUXCMD
. The error in the }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\Cref{eq:supadjsys_prop}}\hspace{0pt}%DIFAUXCMD
, and where
%DIF > Let $\Lambda$ be the solution of the dual problem, \red{$\mathcal{M}'$ has just one '?}
%DIF > 
%DIF > \begin{equation}
%DIF > \mathcal{M}_{HF}'(\Psi_{LF};\Phi, \Lambda)=\mathcal{Q}(\Phi),\quad\forall\Phi\in(Q_{HF}\times U_{HF}\times U_{HF})^2,
%DIF > \end{equation}
%DIF > 
where $\mathcal{Q}:(U_{HF} \times U_{HF} \times Q_{HF})^2 \to \R$ is a linear functional defined by,
%DIF > 
}\begin{equation}
\DIFadd{\mathcal{Q}(\Phi)=\mathcal{M}'_{HF}(\Psi_{LF})(\Phi).
\label{eq:supadjout}
}\end{equation}
%DIF > 
\DIFadd{Then we have,
...
\DIFadd{\label{eq:finErrExp}
I(q_{HF},u_{HF})-I(q_{LF},u_{LF})=}\\\DIFadd{-\frac{1}{2}\mathcal{M}'_{HF}(\Psi_{LF})(\Lambda)+\mathcal M_{HF}(\Psi_{LF})-\mathcal M_{LF}(\Psi_{LF}) + \mathcal{R}_{\mathscr{R}} + \mathcal{R}
}\end{multline}
%DIF > 
\DIFadd{where $\mathcal{R}_{\mathscr{R}}$ is a second-order term in the error $e=\Psi_{HF}-\Psi_{LF}$, and $\mathcal{R}$ is as defined in \mbox{%DIFAUXCMD
\Cref{thm:error_estimate}}\hspace{0pt}%DIFAUXCMD
. %DIF > \red{Now that we've explicitly pointed out the second-order error term, can we really say that we derived "an adjoint-based third-order estimate" (in abstract and intro)?}
%DIF > 
}\end{proposition}
%DIF > 
\begin{proof}
%DIF > 
\DIFadd{The proof simply follows from the classic dual-based a posteriori error estimation strategy. Note that, as a stationary point, $\Psi_{HF}$ satisfies,
%DIF > 
}\begin{equation}
\DIFadd{\mathscr{R}(\Psi_{HF})(\Phi)=0,\quad\forall\Phi\in(Q_{HF}\times U_{HF}\times U_{HF})^2
\label{eq:supadjsys}
}\end{equation}
%DIF > 
\DIFadd{which is represents the equation $\mathcal{M}'_{HF}(\Psi_{HF})(\Phi)=0$ in residual form, where $\Phi$ is a test function. Define the adjoint problem
%DIF > 
}\begin{equation}
\DIFadd{\mathscr{R}'(\Psi_{LF},\Phi)(\Lambda)=\mathcal{Q}(\Phi),\quad\forall\Phi\in(Q_{HF}\times U_{HF}\times U_{HF})^2
\label{eq:superAdjEq}
}\end{equation}
%DIF > 
\DIFadd{for the supplementary adjoint $\Lambda$. The error in the linear }\DIFaddend output $\mathcal{Q}$ defined in \Cref{eq:supadjout} can then be expressed as a dual-weighted residual,
\DIFdelbegin \begin{displaymath}
\DIFdel{%DIFDELCMD < \label{eq:adjOutErr}%%%
\mathcal M'_{HF,\Psi}(\Psi_{LF})(\Psi_{HF}-\Psi_{LF})=-\mathcal{M}'_{HF,\Psi}(\Psi_{LF})(\Lambda).
}\end{displaymath}
%DIFAUXCMD
\DIFdelend %
\DIFdelbegin \DIFdel{Combining \mbox{%DIFAUXCMD
\Cref{eq:preadj} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\Cref{eq:adjOutErr} }\hspace{0pt}%DIFAUXCMD
, we have,
}\begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:finErrExp}%%%
I(q_{HF},u_{HF})-I(q_{LF},u_{LF})=}\\\DIFdel{-\frac{1}{2}\mathcal{M}'_{HF,\Psi}(\Psi_{LF})(\Lambda)+\mathcal M_{HF}(\Psi_{LF})-\mathcal M_{LF}(\Psi_{LF})+\mathcal{R}(e^3).
}\end{align*}
%DIFAUXCMD
%DIF < 
\DIFdel{Other approaches }\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{\label{eq:adjOutErr}
\mathcal{Q}(\Psi_{HF}-\Psi_{LF})=-\mathscr{R}(\Psi_{LF})(\Lambda) + R_{\mathscr{R}},
}\end{equation}
%DIF > 
\DIFadd{where $R_{\mathscr{R}}$ is a second order remainder term due to the linearization of $\mathscr{R}$ about $\Psi_{LF}$ in \mbox{%DIFAUXCMD
\Cref{eq:superAdjEq}}\hspace{0pt}%DIFAUXCMD
. This implies the required result. %DIF > \red{What is the classical notation for adjoint problems? Is it okay that we're breaking from the previous notation where a second pair of parentheses denotes things that we are linear in?}
%DIF > 
}\end{proof}
%DIF > 
\DIFadd{We note that if the state equations of the high-fidelity model are linear, the remainder term $R_\mathscr{R}$ in the supplementary adjoint formulation in \mbox{%DIFAUXCMD
\Cref{eq:adjOutErr} }\hspace{0pt}%DIFAUXCMD
vanishes, and the error estimate in \mbox{%DIFAUXCMD
\Cref{eq:finErrExp} }\hspace{0pt}%DIFAUXCMD
becomes fully third-order. This occurs even if the QoI functional $I$ is nonlinear.
%DIF > If the state equations of the high fidelity model are linear, the derivative of the Lagrangian in~\Cref{eq:InvsOpt_auglag} is linear. In this case, the $R_{\mathscr{R}}$ term in~\Cref{eq:finErrExp} disappears, even if the QoI functional $I$ is nonlinear, and we get a fully third order error estimate.
}

\DIFadd{Other approaches to estimating the term $\frac{1}{2}\mathcal{M}'_{HF}(\Psi_{LF})(\Psi_{HF}-\Psi_{LF})$ }\DIFaddend can become feasible in certain scenarios; for example, if the high- and low-fidelity models only differ in the computational grid used, the difference $\Psi_{HF}-\Psi_{LF}$ can be approximated using interpolation or patch recovery methods~\cite{BecVex05}.
...
Just as error estimates can be used to guide mesh refinement~\cite{BecRann01,VendDarm00,PrudOden99,BecVex05,AinsOden11}, the error estimate \DIFaddbegin \DIFadd{obtained after dropping the remainder terms $\mathcal{R}$ and $\mathcal{R}_\mathscr{R}$ in }\DIFaddend \Cref{eq:finErrExp} can be localized to give elemental contributions and used to guide physics-based refinement over the domain to create a mixed-fidelity model. After refinement, the error estimate can be calculated again, using the mixed-fidelity model as the lower-fidelity model. This process can be repeated, successively increasing the proportion of the domain in which the high-fidelity model is used, until some error threshold is reached. \Cref{alg:refSeries} describes this approach. 
...
\Procedure{$\texttt{BuildMixedFidelity}$}{HF model, LF model, \texttt{errTol}, \texttt{maxIter}}
	...
	\DIFdelbegin %DIFDELCMD < \State{Compute QoI error estimate
%DIFDELCMD < 		

%DIFDELCMD < 	$\epsilon_0=-\frac{1}{2}\mathcal{M}'_{HF,\Psi}(\Psi_{MF_0})(\Lambda_0)+\mathcal M_{HF}(\Psi_{MF_0})-\mathcal M_{MF_0}(\Psi_{MF_0})$}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \State{Compute QoI error estimate

	$\epsilon_0=-\frac{1}{2}\mathcal{M}'_{HF}(\Psi_{MF_0})(\Lambda_0)+\mathcal M_{HF}(\Psi_{MF_0})-\mathcal M_{MF_0}(\Psi_{MF_0})$}
	\DIFaddend \State{Calculate QoI $I(q_{MF_0},u_{MF_0})$}
	\While{$i<$ \texttt{maxIter} and $|\epsilon_i/(\epsilon_i+I(q_{MF_i},u_{MF_i}))|>$ \texttt{errTol}}
		\DIFdelbegin %DIFDELCMD < \State{\begin{varwidth}[t]{\linewidth}Localize $\epsilon_i$ (see \Cref{sec:errLocal}) and use this decomposition to guide \par\hskip\algorithmicindent formation of new mixed-fidelity model MF$_{i+1}$\end{varwidth}}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \State{Localize $\epsilon_i$ (see \Cref{sec:errLocal}) and use this decomposition to guide \par\hskip\algorithmicindent formation of new mixed-fidelity model MF$_{i+1}$}
		\DIFaddend \State{$i\gets i+1$}
...
%DIFDELCMD < 		$\quad\quad \epsilon_i=-\frac{1}{2}\mathcal{M}'_{HF,\Psi}(\Psi_{MF_i})(\Lambda_i)+\mathcal M_{HF}(\Psi_{MF_i})-\mathcal M_{MF_i}(\Psi_{MF_i})$}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \State{Compute QoI error estimate
		...
		\DIFaddend \State{Calculate QoI $I(q_{MF_i},u_{MF_i})$}
	\EndWhile \\
...
\DIFaddbegin \DIFadd{As an example, consider a pair of low- and high-fidelity models described by the convection-diffusion and convection-diffusion-reaction equations
}\begin{subequations}
\begin{align}
\DIFadd{\textrm{low-fidelity:   } }& \DIFadd{k_d\nabla^2 u - \vec{V}\cdot\nabla u = q, \label{eq:cdvcdrLF}}\\
\DIFadd{\textrm{high-fidelity:   } }& \DIFadd{k_d\nabla^2 u - \vec{V}\cdot\nabla u + k_ru^2 = q, \label{eq:cdvcdrHF}
}\end{align}
\end{subequations}
\DIFadd{where $k_d$ and $k_r$ are diffusion and reaction coefficients, respectively, and the state $u$ and parameters $q$ are continuous functions over a domain $\Omega$ with homogenous Dirichlet boundary conditions. To form the mixed-fidelity models, we divide the domain into complementary subdomains, $\Omega_{HF}$ and $\Omega_{LF}$, where the high- and low-fidelity models are solved, respectively; initially, $\Omega_{LF}=\Omega$. The resulting mixed-fidelity models can be described by,
%DIF > 
}\begin{equation}
\DIFadd{k_d\nabla^2 u - \vec{V}\cdot\nabla u + k^{MF}_ru^2= q,
}\end{equation}
%DIF > 
\DIFadd{where $k^{MF}_r$ is a piecewise-constant reaction coefficient, %DIF > that's zero in the $\Omega_{LF}$ and equal to $k_r$ in $\Omega_{HF}$; 
%DIF > 
}\begin{equation}
\DIFadd{k^{MF}_r=
\begin{cases}
...
%DIF > 
\DIFadd{Let the QoI be described by $I(q,u)=\int_{\Omega_I} u \:\textrm{d}\Omega$, where $\Omega_I\subset\Omega$ is the QoI region. Let the observations be of the state at some set of locations, so that the data-misfit term is $\frac{1}{2}\int_\Omega \delta_{obs}(u-d)^2\:\textrm{d}\Omega$, where $\delta_{obs}$ is a sum of Dirac delta functions indicating the locations of the observations. Since the inverse problem is ill-posed, we use Tikhonov regularization~\mbox{%DIFAUXCMD
\cite{EngHanNeu00}}\hspace{0pt}%DIFAUXCMD
, with $R(q)=\frac{\beta}{2}\int_\Omega \|\nabla q\|_2^2\:\textrm{d}\Omega$, where $\beta$ is a regularization coefficient. Assuming divergence-free velocity $\vec{V}$, the mixed-fidelity model has augmented Lagrangian
}\begin{eqnarray}
\DIFadd{\mathcal{M}_{MF}((q,u,z)(p,v,y)) }&\DIFadd{=}& \DIFadd{\int_{\Omega_I\subset\Omega} u \:\textrm{d}\Omega \nonumber}\\
&\DIFadd{+}& \DIFadd{\int_\Omega p(-\beta\nabla^2q+z) \nonumber}\\
&\DIFadd{+}& \DIFadd{v(k_d\nabla^2z+\vec{V}\cdot\nabla z+2k^{MF}_rzu+\delta_{obs}(u-d)) \nonumber}\\ 
&\DIFadd{+}& \DIFadd{y(k_d\nabla^2u-\vec{V}\cdot\nabla u+k^{MF}_ru^2+q)\:\textrm{d}\Omega.
}\end{eqnarray}
\DIFadd{Taking variations with respect to the auxiliary variables $(p,v,y)$ recovers the equations for the primary variables; taking variations with respect to the primary variables gives the equations for the auxiliary variables. Solving for the primary variables and then the auxiliary variables gives the stationary point $\Psi_{MF}$ of $\mathcal{M}_{MF}$. 
The high-fidelity versions of these primary and auxiliary systems are satisfied by $\Psi_{HF}$; we seek the error in the linear functional described by \mbox{%DIFAUXCMD
\Cref{eq:supadjout} }\hspace{0pt}%DIFAUXCMD
induced by applying it to $\Psi_{MF}$ instead of $\Psi_{HF}$. We take the classical dual-weighted residual approach and solve for the supplementary adjoint $\Lambda$, linearizing about $\Psi_{MF}$ instead of $\Psi_{HF}$ when necessary, since the later is not available; the components of the supplementary adjoint corresponding to the primary and auxiliary systems can be solved separately. Having obtained $\Psi_{MF}$ and the supplementary adjoint $\Lambda$, we evaluate the error estimate (see Step \ref{alg-line:MFerrorest} of \mbox{%DIFAUXCMD
\Cref{alg:refSeries}}\hspace{0pt}%DIFAUXCMD
), localize it (see \mbox{%DIFAUXCMD
\Cref{sec:errLocal}}\hspace{0pt}%DIFAUXCMD
), and use the localization to guide the selection of elements currently in $\Omega_{LF}$ to include in $\Omega_{HF}$ in the next iteration. We return to this example and give results of applying \mbox{%DIFAUXCMD
\Cref{alg:refSeries} }\hspace{0pt}%DIFAUXCMD
to this pair of models in \mbox{%DIFAUXCMD
\Cref{sec:cdvcdr}}\hspace{0pt}%DIFAUXCMD
.
%DIF > The weighted residual of the high-fidelity primary and auxiliary equations, evaluated at $\Psi_{MF}$, describes a linear functional in the weights (see \Cref{eq:supadjout}). Taking the adjoint of these high-fidelity primary and auxiliary systems, with output described by this linear functional, gives the supplementary adjoint; the components of the supplementary adjoint corresponding to the primary and auxiliary systems can be solved separately. Having obtained $\Psi_{MF}$ and the supplementary adjoint $\Lambda$, we evaluate the error estimate (see Step \ref{alg-line:MFerrorest} of \Cref{alg:refSeries}), localize it (see \Cref{sec:errLocal}), and use the localization to guide the selection of elements currently in $\Omega_{LF}$ to include in $\Omega_{HF}$ in the next iteration. We return to this example and give results of applying \Cref{alg:refSeries} to this pair of models in \Cref{sec:cdvcdr}.
}

\DIFaddend \Cref{alg:refSeries} is applicable to a large class of models. The lower-fidelity model could, for example, be a simplified model including fewer physical phenomena, be a reduced-order model, or have a reduced parameter \DIFaddbegin \DIFadd{and/or state }\DIFaddend space. The \DIFdelbegin \DIFdel{two models could also correspond to two levels of mesh refinement, though in this case the method described in~\mbox{%DIFAUXCMD
\cite{BecVex05} }\hspace{0pt}%DIFAUXCMD
would likely be more efficient, since interpolation could be used to estimate $\Psi_{HF}-\Psi_{LF}$ instead. The }\DIFdelend derived error estimate is not applicable to all models, however. \DIFdelbegin \DIFdel{The two models have to be expressed }\DIFdelend \DIFaddbegin \DIFadd{It is assumed that the two models share the same domain; for example, the low-fidelity model can not be a one-dimensional average of a two-dimensional high-fidelity model. The low- and high-fidelity models must by expressible }\DIFaddend in a weak form, so this cannot be applied to, for example, a model of chemical reactions using kinetic Monte Carlo. We need some degree of compatibility between the two models\DIFdelbegin \DIFdel{; namely, we assume }\DIFdelend \DIFaddbegin \DIFadd{, in that we require the low-fidelity solutions to be interpretable in the context of the high-fidelity model; specifically, we require }\DIFaddend that $\Psi_{LF}$ \DIFdelbegin \DIFdel{will }\DIFdelend be in a space admissible to \DIFdelbegin \DIFdel{$\mathcal{M}'_{HF,\Psi}$, }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{M}'_{HF}$ }\DIFaddend and \DIFdelbegin \DIFdel{that the QoI functional $I$ is applicable to both $(q_{HF},u_{HF})$ and $(q_{LF},u_{LF})$.
}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{M}_{HF}$. For example, compared to the high-fidelity model, low-fidelity model may have a coarser mesh, basis functions of a lower order, or basis functions spanning a reduced subspace. The high-fidelity model could describe changes in states that have a fixed nominal value in the low-fidelity model; the high-fidelity model could allow anisotropy of the inferred parameters while the low-fidelity model could constrain their components and require isotropy. We note that in the case where the two models correspond to two levels of mesh refinement, method described in~\mbox{%DIFAUXCMD
\cite{BecVex05} }\hspace{0pt}%DIFAUXCMD
would likely be more efficient, since interpolation could be used to estimate $\Psi_{HF}-\Psi_{LF}$ instead.
%DIF > , and that the QoI functional $I$ is applicable to both $(q_{HF},u_{HF})$ and $(q_{LF},u_{LF})$. %implied by admissibility of LF to HF
}\DIFaddend 
...
\DIFaddend \Cref{alg:refSeries} does not require a specific method for localizing the error estimate. A na\"{i}ve approach would be to write the error estimate as a sum of integrals over elements and their boundaries, and calculate the error contribution by each element as the integral over that element. While simple, this method can lead to non-zero error contributions from elements in which the high-fidelity model is already being used, making the error decomposition more difficult to interpret and use for refinement.

We instead use the alternative method described in \cite{vanOpstaletal15}, decomposing the error estimate into contributions from locally supported basis functions rather than elements. \DIFdelbegin \DIFdel{For ease of implementation, we can choose these basis functions to be the same as those spanning the space }\DIFdelend \DIFaddbegin \DIFadd{Recall the error estimate 
%DIF > 
}\begin{equation}
\DIFadd{\epsilon=-\frac{1}{2}\mathcal{M}'_{HF}(\Psi_{MF})(\Lambda)+\mathcal M_{HF}(\Psi_{MF})-\mathcal M_{MF}(\Psi_{MF})
}\end{equation}
%DIF > 
\DIFadd{from \mbox{%DIFAUXCMD
\Cref{alg:refSeries}}\hspace{0pt}%DIFAUXCMD
. Let $(Q^h\times U^h\times U^h)^3 \subset (Q\times U\times U)^3$ be the finite-dimensional conforming subspace }\DIFaddend in which we solve for the \DIFdelbegin \DIFdel{primary, auxiliary,
and supplementary adjoint variables}\DIFdelend \DIFaddbegin \DIFadd{approximations $\Lambda^h$ and $\Psi_{MF}^h$; we seek to decompose the error estimate
%DIF > 
}\begin{equation}
\DIFadd{\epsilon^h=-\frac{1}{2}\mathcal{M}'_{HF}(\Psi_{MF}^h)(\Lambda^h)+\mathcal M_{HF}(\Psi_{MF}^h)-\mathcal M_{MF}(\Psi_{MF}^h).
}\end{equation}
%DIF > 
\DIFadd{Define a basis $\Phi^h=\{(\varphi,\upvarphi)_i\}_{i\in I}$ consisting of locally supported functions such that $\textrm{span}(\Phi^h)=(Q^h\times U^h\times U^h)^3$, so that we can write $(\Lambda^h,\Psi_{MF}^h)=(\sum\limits_{i\in I}\varphi_i\lambda_i,\sum\limits_{i\in I}\upvarphi_i \psi_i)$. Then the error estimate satisfies
%DIF > 
}\begin{equation}
\DIFadd{\epsilon^h \leq \sum_{i\in I} \varepsilon^h_i,
}\end{equation}
%DIF > 
\DIFadd{where,
%DIF > 
}\begin{equation}\DIFadd{\label{eq:basisblame}
...
%------------------------------------------------------------%
%
We consider a rectangular domain $\Omega(x_1,x_2)=[0,5]\times[0,1]$, where $x_1$ and $x_2$ are the spatial coordinates. \DIFaddbegin \DIFadd{Recall the pair of models introduced in \mbox{%DIFAUXCMD
\Cref{sec:adapt_alg}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend The high-fidelity model is a single-species convection-diffusion-reaction equation with a nonlinear reaction term, described by \DIFdelbegin \DIFdel{,
%DIF < 
}%DIFDELCMD < \begin{subequations}
%DIFDELCMD < \label{eq:cdvcdrHF}
%DIFDELCMD < %%%
\begin{align*}
\DIFdel{k_d\nabla^2 u - \vec{V}\cdot\nabla u + k_ru^2 = f(q) \quad }&\DIFdel{\text{in } \Omega,  }\\
\DIFdel{u = 0 \quad }&\DIFdel{\text{on } \partial \Omega, 
}\end{align*}
%DIFAUXCMD
%DIFDELCMD < \end{subequations}
%DIFDELCMD < %%%
%DIF < 
\DIFdel{where the state $u$ is the species concentration and $f(q)$ is a forcing field described by the parameters}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\Cref{eq:cdvcdrHF}}\hspace{0pt}%DIFAUXCMD
. The low-fidelity model, described by \mbox{%DIFAUXCMD
\Cref{eq:cdvcdrLF}}\hspace{0pt}%DIFAUXCMD
, differs only in the removal of the reaction term}\DIFaddend . We have a divergence-free parabolic-profile velocity field $\vec{V}(x_1,x_2) = (2x_2(1-x_2),0)$; the diffusion and reaction coefficients are $k_d = 0.1$ and $k_r = -42.0$, respectively. \DIFdelbegin \DIFdel{The low-fidelity model,
%DIF < 
}\begin{displaymath}
\DIFdel{k_d\nabla^2 u - \vec{V}\cdot\nabla u = f(q)
}\end{displaymath}
%DIFAUXCMD
%DIF < 
\DIFdel{differs only in the removal of the reaction term. To form the mixed-fidelity models, we divide the domain into complementary subdomains, $\Omega_{HF}$ and $\Omega_{LF}$, where the high- and low-fidelity models are solved, respectively. The resulting mixed-fidelity models can be described by,
%DIF < 
}\begin{displaymath}
...
%DIFAUXCMD
%DIF < 
\DIFdel{where $k^{MF}_r$ is a piecewise-constant reaction coefficient,
%DIF < 
}\begin{displaymath}
\DIFdel{k^{MF}_r=
\begin{cases}
...
%DIFAUXCMD
%DIF < 
\DIFdelend Homogeneous Dirichlet boundary conditions are applied on the entire boundary of the domain. 
\DIFdelbegin \DIFdel{The QoI we wish to calculate is the integral of the state,
%DIF < 
}\begin{displaymath}
\DIFdel{I(q,u)=\int_{(x_1,x_2)\in \Omega_I} u \:\textrm{d}A,
}\end{displaymath}
%DIFAUXCMD
%DIF < 
\DIFdel{over a region }\DIFdelend \DIFaddbegin 

\DIFadd{The QoI region is }\DIFaddend $\Omega_I=[0.625,0.875]\times[0.375,0.625]$. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The unknown parameters to be inferred }\DIFdelend \DIFaddbegin \DIFadd{The parameters $q$ we wish to infer }\DIFaddend correspond to the forcing \DIFdelbegin \DIFdel{field, so that in this case $f(q)=q$}\DIFdelend \DIFaddbegin \DIFadd{function}\DIFaddend . The observations are sparse measurements of the state.
\Cref{fig:baseSetup} shows the locations of the observations and the region $\Omega_I$ over which the QoI is calculated. \DIFdelbegin \DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{Note that for }\DIFaddend the low-fidelity model, the inverse problem is linear (i.e., the observations depend linearly on the parameters). \DIFdelbegin \DIFdel{Since the inverse problem is ill-posed, we use Tikhonov regularization ~\mbox{%DIFAUXCMD
\cite{EngHanNeu00}}\hspace{0pt}%DIFAUXCMD
. The regularization term in the objective function \mbox{%DIFAUXCMD
\Cref{eq:invOpt_obj} }\hspace{0pt}%DIFAUXCMD
is $R(q)=\frac{\beta}{2}\int_\Omega \|\nabla f(q)\|_2^2\:\textrm{d}A$, where }\DIFdelend \DIFaddbegin \DIFadd{We arbitarily choose the regularization coefficient to be }\DIFaddend $\beta=10^{-5}$\DIFdelbegin \DIFdel{is the regularization coefficient }\DIFdelend ; in this work, $\beta$ is chosen arbitrarily, though methods exist for choosing optimal regularization coefficients~\cite{BelgeKilmerMiller02,Calvetal00}.
%
\begin{figure}[htbp]
\centering
...
For the numerical simulations, we use the finite element method (FEM), employing a continuous Galerkin formulation with \DIFaddbegin \DIFadd{first-order }\DIFaddend Lagrange elements. We use the \texttt{libMesh} library~\cite{libMeshPaper} for the FEM calculations.
...
The domain is discretized by a regular mesh of quadrilaterals, with 250 and 50 elements along the $x_1$ and $x_2$ directions, respectively, for a total of 12,500 elements, resulting in 12,801 degrees of freedom per variable\DIFaddbegin \DIFadd{; all variables are represented by the same basis}\DIFaddend . The diffusion coefficient is such that the cell P\'{e}clet number never exceeds 0.1, and thus no stabilization is required.
...
\begin{figure}[htbp]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\textwidth]{baseSeries/err_est.pdf}
%DIFDELCMD < %%%
\DIFdelendFL %DIF > \includegraphics[width=0.8\textwidth]{baseSeries/err_est.pdf}
\DIFaddbeginFL \includegraphics[width=0.8\textwidth]{baseSeries/err_est_v2.pdf}
%DIF > \caption{True and estimated absolute relative error in QoI, plotted as a function of the percentage area of the domain in which the high-fidelity convection-diffusion-reaction model is used.}
\DIFaddendFL \caption{True and estimated absolute relative error in QoI, plotted as a function of the percentage area of the domain in which the high-fidelity convection-diffusion-reaction model is used. \DIFaddbeginFL \DIFaddFL{In this case, only two refinement iterations are necessary to achieve the target relative QoI error (dotted line).}\DIFaddendFL }
\label{fig:baseErr}
\end{figure}
%
...
\Cref{fig:qoiStudy} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\Cref{subfig:obsSetup} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend shows the domain for three cases considered, with the same set of observations but increasingly large, nested QoI regions $\Omega_I$.
The error decompositions for each case are \DIFdelbegin \DIFdel{also shown in \mbox{%DIFAUXCMD
\Cref{fig:qoiStudy}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{shown in \mbox{%DIFAUXCMD
\Cref{subfig:obsLF,subfig:obsMF1,subfig:obsMFlast}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The bottom row gives the baseline case presented in \Cref{sec:cdvcdrBaseRef}, although here we choose the basis functions $i$ whose error $\varepsilon_i$ are among the largest $5\%$, rather than only enough basis functions to cover 5\% of the domain in their support, so the proportion of elements marked for refinement in each iteration will be slightly larger than in \Cref{sec:cdvcdrBaseRef}.
...
\captionsetup{justification=centering}
\DIFdelbeginFL %DIFDELCMD < \subfloat[MF$_1$ ($\sim5\%$ HF)][MF$_1$ \\($\sim5\%$ HF)]{
%DIFDELCMD <   %\includegraphics[width=0.23\textwidth]{vs_qoi/vs_qoi_err1.png}
%DIFDELCMD <   \includegraphics[width=0.16\textwidth]{vs_qoi/vs_qoi_err1_nobar.png}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[MF$_1$ ($\sim5\%$ HF)][MF$_1$ \\($\sim5\%$ HF)]{
...
\DIFaddendFL \captionsetup{justification=centering}
...
%DIFDELCMD <   \includegraphics[width=0.16\textwidth]{vs_data/vs_data_err1_nobar.png}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[MF$_1$ ($\sim5\%$ HF)][MF$_1$ \\($\sim5\%$ HF)]{
...
differs from the high-fidelity model only in that \DIFdelbegin \DIFdel{the parameter $q$ is a constant instead of a field}\DIFdelend \DIFaddbegin \DIFadd{$U_{LF}$ is limited to the space of constant functions over $\Omega$; we maintain $f(q)=q$}\DIFaddend . The intermediate mixed-fidelity models thus have parameter fields \DIFaddbegin \DIFadd{(and forcing functions) }\DIFaddend that are constant over the subregions of the domain where the low-fidelity model is used\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{; disconnected low-fidelity regions still share the same parameter value. The domain, mesh, boundary conditions, and velocity field, as well as the observations and QoI, remain the same as described in \mbox{%DIFAUXCMD
\Cref{sec:cdvcdr}}\hspace{0pt}%DIFAUXCMD
; we continue to infer for the forcing field. }\DIFaddend For ease of implementation, we require that the resulting parameter field remain continuous at the interface between the low-fidelity and high-fidelity subdomains, although this constraint is not necessary for the theory to hold. \DIFdelbegin \DIFdel{The domain, mesh, boundary conditions, and velocity field, as well as the observations, unknown parameters to be inferred, and QoI, remain the same as described in \mbox{%DIFAUXCMD
\Cref{sec:cdvcdr}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Also for ease of implementation and for localizing the error, the forcing field and its auxiliary and supplementary adjoint conterparts are represented with the same basis functions as the other variables in the high-fidelity regions, and forced to be constant in the low-fidelity regions using a penalty term. Although the chosen pair of models has the potential to give mixed-fidelity models with fewer degreees of freedom in the parameter, this is not the case in our implementation; however, we are not looking to demonstrate computational savings with this example, but rather to explore the behavior of \mbox{%DIFAUXCMD
\Cref{alg:refSeries} }\hspace{0pt}%DIFAUXCMD
when applied to such a pair of models}\DIFaddend . As the inverse problem is ill-posed, except when the low-fidelity model is used throughout the domain, regularization is used; the Tikhonov regularization term in \Cref{eq:invOpt_obj} is $R(q)=\frac{\beta}{2}\int_\Omega \|\nabla f(q)\|_2^2+f(q)^2\:\textrm{d}A$, where $\beta=10^{-3}$ is a regularization coefficient. 
...
\Cref{fig:svfRefParams} }\hspace{0pt}%DIFAUXCMD
...
\caption{\DIFaddFL{Inferred parameters using mixed-fidelity (}\protect\subref{subfig:svfparamMF1}\DIFaddFL{--}\protect\subref{subfig:svfparamMF6}\DIFaddFL{) and high-fidelity (}\protect\subref{subfig:svfparamHF}\DIFaddFL{) models. All figures share the color-scale for the high-fidelity plot.}}
\label{fig:svfRefParams}
\end{figure}
%DIF > 
\DIFaddend The corresponding true and estimated absolute errors in the QoI are shown in \Cref{fig:svfErr}.
%
\begin{figure}[htbp]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\textwidth]{svf/err_est.pdf}
%DIFDELCMD < %%%
\DIFdelendFL %DIF > \includegraphics[width=0.8\textwidth]{svf/err_est.pdf}
\DIFaddbeginFL \includegraphics[width=0.8\textwidth]{svf/err_est_v2.pdf}
\DIFaddendFL \caption{True and estimated absolute relative error in QoI, plotted as a function of the percentage area of the domain in which the high-fidelity field-parameter model is used. \DIFaddbeginFL \DIFaddFL{The dotted line indicates the target relative QoI error.}\DIFaddendFL }
\label{fig:svfErr}
\end{figure}
%
In this case, we see that we must use the high-fidelity model in most of the domain in order to get an accurate QoI. The adaptive algorithm requires us to use the field representation of the high-fidelity model in much of the left half of the domain; this reflects the topology of the inferred parameter field in the high-fidelity inverse problem, which is only relatively constant towards the right portion of the domain \DIFaddbegin \DIFadd{(compare \mbox{%DIFAUXCMD
\Cref{subfig:svfparamMF6} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\Cref{subfig:svfparamHF}}\hspace{0pt}%DIFAUXCMD
)}\DIFaddend . We also see that in this case, in contrast to the example in \Cref{sec:cdvcdrBaseRef}, increasing the proportion of the domain in which the high-fidelity model is used does not monotonically decrease the error in the QoI. \DIFaddbegin \DIFadd{Note that although the inferred parameters in the first and third mixed-fidelity models (see \mbox{%DIFAUXCMD
\Cref{subfig:svfparamMF1} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\Cref{subfig:svfparamMF3}}\hspace{0pt}%DIFAUXCMD
) are very different, they give very similar QoI error; the same is true for the second and fourth mixed-fidelity models. These provide additional examples supporting the idea that using inferring parameters with models of different mixtures of fidelities can produce similar QoIs.
}\DIFaddend 

%DIF > compare MF01 and MF03, MF06 and HF
%DIF > 1 and 3 look very different but have very similar true errors - different params can give similar qoi, though in this case the qoi is not close to HF; more HF doesn't necessarily decrease error, interplay between having freedom to match data and all LF regions being flat...
%DIF > 6 and HF look very similar; most of HF is very flat
\DIFaddbegin 
...
The unknown parameters we wish to infer correspond to the source term $f(q)=q$; we impose $f(q)=q=0$ on the boundary $\partial\Omega$. Synthetic observations at 18 points in the domain are generated by running the high-fidelity model on a finer mesh. The locations of the observations as well as the QoI region \DIFdelbegin \DIFdel{$\Omega_I$ }\DIFdelend \DIFaddbegin \DIFadd{$\Omega_I=[1100\textrm{m},1200\textrm{m}]\times[775\textrm{m},875\textrm{m}]\times[80\textrm{m},100\textrm{m}]$ }\DIFaddend are shown in \Cref{fig:setup3D}. We set the regularization term in \Cref{eq:invOpt_obj} to be $R(q)=\frac{\beta}{2}\int_\Omega \|\nabla f(q)\|_2^2\:\textrm{d}V$.
%
\begin{figure}[htbp]
\centering
...
%
\begin{table}[htbp]
\DIFdelbeginFL %DIFDELCMD < \label{tab:ref3D_diffmesh}
%DIFDELCMD < %%%
...
The multi-fidelity domain refinements for the five adaptive iterations are shown in \Cref{fig:divvy3D_diffmesh}. Similarly to the behavior seen in \Cref{sec:cdvcdrBaseRef}, the QoI region and the areas around some of the measurement points are first targeted for refinement, with those measurement points furthest downstream of the QoI being the last to receive refinement. We also see that the domain is refined completely in the $x_3$ direction first \DIFdelbegin \DIFdel{around }\DIFdelend \DIFaddbegin \DIFadd{in an area slightly upstream of }\DIFaddend the QoI region \DIFaddbegin \DIFadd{(see \mbox{%DIFAUXCMD
\Cref{subfig:3DMF2}}\hspace{0pt}%DIFAUXCMD
)}\DIFaddend , reflecting the large difference in the high-fidelity dispersion tensor $D$ and the low-fidelity dispersion coefficient in the $x_3$ direction.
%
\begin{figure}[htbp]
\centering
\DIFdelbeginFL %DIFDELCMD < \subfloat[MF$_1$ ($5.2\%$ HF)]{
%DIFDELCMD < \includegraphics[width=0.31\textwidth]{series3D/run_diff_mesh/divvy1_whitebg_puff.png}
%DIFDELCMD < }
%DIFDELCMD < \subfloat[MF$_2$ ($11.4\%$ HF)]{
%DIFDELCMD < \includegraphics[width=0.31\textwidth]{series3D/run_diff_mesh/divvy2_whitebg_puff.png}
%DIFDELCMD < }
%DIFDELCMD < \subfloat[MF$_3$ ($16.3\%$ HF)]{
%DIFDELCMD < \includegraphics[width=0.31\textwidth]{series3D/run_diff_mesh/divvy3_whitebg_puff.png}
%DIFDELCMD < } %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Slice location and color key]{
...
\DIFaddendFL \\
\DIFdelbeginFL %DIFDELCMD < \subfloat[MF$_4$ ($22.0\%$ HF)]{
%DIFDELCMD < \includegraphics[width=0.31\textwidth]{series3D/run_diff_mesh/divvy4_whitebg_puff.png}
%DIFDELCMD < }
%DIFDELCMD < \subfloat[MF$_5$ ($27.7\%$ HF)]{
%DIFDELCMD < \includegraphics[width=0.31\textwidth]{series3D/run_diff_mesh/divvy5_whitebg_puff.png}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[MF$_2$ ($11.4\%$ HF)]{
...
\DIFaddendFL \caption{Domain division for mixed-fidelity models \DIFaddbeginFL \DIFaddFL{(}\protect\subref{subfig:3DMF1}\DIFaddFL{--}\protect\subref{subfig:3Dfinref}\DIFaddFL{)}\DIFaddendFL : \DIFdelbeginFL \DIFdelFL{low-fidelity }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3D view with 2D slice through domain below (see }\protect\subref{subfig:3Dsliceloc} \DIFaddFL{for slice location and orientation). Low-fidelity }\DIFaddendFL convection-diffusion model used in red portion, high-fidelity convection-diffusion-reaction model used in blue portion (intermediate colors \DIFaddbeginFL \DIFaddFL{in 3D view }\DIFaddendFL due to transparency indicate a mix of the two models along the line of sight)\DIFdelbeginFL \DIFdelFL{; }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. Slices cut through QoI region, indicated by purple box. }\DIFaddendFL $x_3$ direction scaled for clarity.}
...
%
\begin{table}
\DIFdelbeginFL %DIFDELCMD < \label{tab:ref3D_newdata_QoI_diffmesh}
%DIFDELCMD < %%%
\DIFdelendFL \centering
...
\DIFaddbeginFL \label{tab:ref3D_newdata_QoI_diffmesh}
\DIFaddendFL \end{table}
...
{%DIFAUXCMD
\DIFdelFL{Average times to solve inverse problem and obtain error estimate with mixed- and high-fidelity models and noisy datasets.}}
%DIFAUXCMD
%DIFDELCMD < \label{tab:ref3D_newdata_times_diffmesh}
%DIFDELCMD < %%%
\DIFdelendFL \centering
...
\end{tabular}
\DIFaddbeginFL \caption{\DIFaddFL{Average times to solve inverse problem and obtain error estimate with mixed- and high-fidelity models and noisy datasets.}}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{thebibliography}{42}
...
\DIFaddbegin \bibitem[{Becker and Vexler(2004)}]{becker2004posteriori}
\DIFadd{Becker, R., Vexler, B., 2004. A posteriori error estimation for finite element
  discretization of parameter identification problems. Numerische Mathematik
  96~(3), 435--459.
}

\DIFaddend \bibitem[{Becker and Vexler(2005)}]{BecVex05}
...
\DIFaddbegin \bibitem[{Oden and Vemaganti(2000)}]{OdenVem00}
\DIFadd{Oden, J.~T., Vemaganti, K.~S., 2000. Estimation of local modeling error and
  goal-oriented adaptive modeling of heterogeneous materials: }{\DIFadd{I}}\DIFadd{. }{\DIFadd{E}}\DIFadd{rror
  estimates and adaptive algorithms. Journal of Computational Physics 164~(1),
  22--47.
}

\DIFaddend \bibitem[{Park(2004)}]{Park04}
...
van Opstal, T., Bauman, P., Prudhomme, S., van Brummelen, E., 2015.
  ...
  Computational Mechanics, 1--10\DIFaddbegin \DIFadd{.
}

\bibitem[{Vemaganti and Oden(2001)}]{VemOden01}
\DIFadd{Vemaganti, K.~S., Oden, J.~T., 2001. Estimation of local modeling error and
  goal-oriented adaptive modeling of heterogeneous materials: }{\DIFadd{P}}\DIFadd{art }{\DIFadd{II}}\DIFadd{: a
