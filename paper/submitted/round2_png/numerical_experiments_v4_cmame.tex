...
For the numerical simulations, we use the finite element method (FEM), employing a continuous Galerkin formulation with linear Lagrange elements. We use the \texttt{libMesh} library~\cite{libMeshPaper} for the FEM calculations.
...
\end{equation}
differs from the high-fidelity model only in that the parameter $q$ is a constant instead of a field; $f(q)$ is a constant function equal to $q$ everywhere in the domain. The intermediate mixed-fidelity models thus have parameter fields (and forcing functions) that are constant over the subregions of the domain where the low-fidelity model is used; disconnected low-fidelity regions still share the same parameter value. For ease of implementation, we require that the resulting parameter field remain continuous at the interface between the low-fidelity and high-fidelity subdomains, although this constraint is not necessary for the theory to hold. The domain, mesh, boundary conditions, and velocity field, as well as the observations and QoI, remain the same as described in \Cref{sec:cdvcdr}; we continue to infer for the forcing field. The forcing field (and its auxiliary and supplementary adjoint conterparts) are represented with the same basis functions as the other variables in the high-fidelity regions, and forced to be constant in the low-fidelity regions. As the inverse problem is ill-posed, except when the low-fidelity model is used throughout the domain, regularization is used; the Tikhonov regularization term in \Cref{eq:invOpt_obj} is $R(q)=\frac{\beta}{2}\int_\Omega \|\nabla f(q)\|_2^2+f(q)^2\:\textrm{d}A$, where $\beta=10^{-3}$ is a regularization coefficient. 
