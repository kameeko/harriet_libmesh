...
\end{equation}
%
Now using the trapezoidal rule for the integral in~\Cref{eq:Mantiderivative}, we further have,
%
\begin{equation}
...
\end{equation}
%
which is finite due to the third order differentiability assumptions in the Proposition. We further note that $\mathcal{M}'_{HF}(\Psi_{HF})(e)$ vanishes since $\Psi_{HF}$ is a stationary point of $\mathcal{M}_{HF}$, and~\Cref{eq:Mprimetrapezoid} can be simplified to,
%
\begin{equation}
\label{eq:beckvex}
...
\begin{proof}
%
The proof simply follows from the classic dual based a posteriori error estimation strategy. Note that, as a stationary point, $\Psi_{HF}$ satisfies~\Cref{eq:supadjsys}. Let the variational form
%
\begin{equation}
