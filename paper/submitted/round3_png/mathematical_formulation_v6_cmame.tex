...
%------------------------------------------------------------------------------%
%
For a given hierarchy of compatible models, we take the QoI calculated from inferring the parameters with the highest-fidelity model to be the `exact' or `true' solution. Compatibility allows one to evaluate residuals associated with the high-fidelity inverse problem, such as optimality conditions, at solutions to the low-fidelity inverse problem. Let compatibility be formally defined as follows:
%
\begin{definition}
The high- and low-fidelity models described by \Cref{eq:invOpt_consHF} and \Cref{eq:invOpt_consLF}, respectively, are \emph{compatible} if $U_{LF}\subseteq U_{HF}$ and $Q_{LF}\subseteq Q_{HF}$. 
\label{defn:comp}
\end{definition}
% 
