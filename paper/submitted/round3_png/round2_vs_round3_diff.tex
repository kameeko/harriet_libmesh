...
%DIF DEL ../round2_png/cleaned_for_submit/paper_v5_cmame.tex   Thu Jul 13 09:49:39 2017
%DIF ADD paper_v6_cmame.tex                                    Mon Oct 23 16:54:03 2017
%%\usepackage{lgrind}
...
%------------------------------------------------------------------------------%
%
For a given hierarchy of \DIFaddbegin \DIFadd{compatible }\DIFaddend models, we take the QoI calculated from inferring the parameters with the highest-fidelity model to be the `exact' or `true' solution\DIFdelbegin \DIFdel{; we }\DIFdelend \DIFaddbegin \DIFadd{. Compatibility allows one to evaluate residuals associated with the high-fidelity inverse problem, such as optimality conditions, at solutions to the low-fidelity inverse problem. Let compatibility be formally defined as follows:
%DIF > 
}\begin{definition}
\DIFadd{The high- and low-fidelity models described by \Cref{eq:invOpt_consHF} and \Cref{eq:invOpt_consLF}, respectively, are \emph{compatible} if $U_{LF}\subseteq U_{HF}$ and $Q_{LF}\subseteq Q_{HF}$. 
}\label{defn:comp}
\end{definition}
%DIF >  
