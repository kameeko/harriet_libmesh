...
nElemHF = 2*nElemLF; %keep to whole multiples of nElemLF
...
umatchtol = 10^-8; %acceptable mismatch between consecutive iterations of u before stopping
qmatchtol = 10^-8; %acceptable mismatch between consecutive iterations of q before stopping

%velocities
Ufd = 2*ones(1,nNodesEx); 
ULF = 2*ones(1,nElemLF+1); 
UHF = 2*ones(1,nElemHF+1); 
Uenr = 2*ones(1,2*nElemLF+1); %for superadjoint
...
ypdiffestexp = sum(reshape(MHxHmNHxMexp,2,nElemLF)',2) + remainauexp + remainaexp;
